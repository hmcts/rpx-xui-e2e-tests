...
export type StorageStateContext = Pick<
  BrowserContext,
  "addCookies" | "storageState"
>;
...
function getAppShellMarkers(
  ...
  markers.push(
    { name: "exui-header", locator: page.locator("exui-header").first() },
    {
      ...
      locator: page.getByRole("link", { name: "Create case" }).first(),
    },
    {
      name: "case-list-link",
      locator: page.getByRole("link", { name: "Case list" }).first(),
    },
    {
      name: "case-action-dropdown",
      locator: page.locator("#next-step").first(),
    },
    {
      name: "jurisdiction-select",
      locator: page.locator("#cc-jurisdiction").first(),
    },
  );
...
export async function ensureAuthenticatedPage(
  ...
  if (options.waitForSelector) {
    ...
    const waitForAppShell = async () => {
      ...
      const marker = await waitForAuthenticatedShell(
        page,
        userIdentifier,
        selectors,
        timeoutMs,
...
async function persistSession(
  ...
  try {
    ...
    const augmented = JSON.parse(
      fsApi.readFileSync(localSessionPath, "utf8"),
    ).cookies;
...
async function loginAndPersistSession({
  ...
  const idamLoginUrl = activeConfig.urls.idamWebUrl
    ? new URL("/login", activeConfig.urls.idamWebUrl).toString()
    : undefined;
  const loginTargets = [targetUrl, idamLoginUrl].filter(
...
async function sessionCaptureWith(
  identifiers: string[],
  deps: SessionCaptureDeps = {},
) {
