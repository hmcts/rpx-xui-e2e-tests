...
export function collectDependencySignals(
  ...
  if (failedRequests.length > 0) {
    const reasons = Array.from(
      new Set(failedRequests.map((request) => request.errorText)),
    ).slice(0, 3);
    signals.push(
      `Failed backend requests=${failedRequests.length}${reasons.length > 0 ? ` (${reasons.join("; ")})` : ""}`,
    );
  }
...
export function classifyFailureCategory(
  ...
  if (
    DEPENDENCY_FAILURE_TYPES.has(failureType) ||
