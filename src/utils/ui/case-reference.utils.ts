...
const CASE_REFERENCE_ENV_FALLBACK_KEYS = [
  "PW_PUBLICLAW_CASE_REFERENCE",
  "PW_CASE_REFERENCE",
  "PW_GLOBAL_SEARCH_CASE_REFERENCE",
  "E2E_PUBLICLAW_CASE_REFERENCE",
  "E2E_CASE_REFERENCE",
] as const;
...
function resolveCaseReferenceFromEnvFallback(): string | undefined {
  for (const key of CASE_REFERENCE_ENV_FALLBACK_KEYS) {
    const value = process.env[key]?.trim();
    if (value && CASE_REFERENCE_REGEX.test(value)) {
      return value;
    }
  }
  return undefined;
...
export async function resolveCaseReferenceFromGlobalSearch( // NOSONAR typescript:S3776
  ...
  if (lastStatus !== 200) {
    ...
    const envFallbackCaseReference = resolveCaseReferenceFromEnvFallback();
    if (envFallbackCaseReference) {
      return envFallbackCaseReference;
    }
    throw new Error(
  ...
  throw new Error(
    `No 16-digit case references returned by global search API. Set one of: ${CASE_REFERENCE_ENV_FALLBACK_KEYS.join(", ")}`,
  );
}
...
export async function resolveNonExistentCaseReference( // NOSONAR typescript:S3776
  ...
  const strictResolutionMode = ["1", "true", "yes"].includes(
    (process.env.PW_STRICT_NONEXISTENT_CASE_REFERENCE ?? "")
      .trim()
      .toLowerCase(),
  );
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    ...
    for (
      ...
      response = await executeGlobalSearchRequest(
        page,
        candidateReference,
        jurisdictionIds,
      );
