...
async function ensureStorageStateWith(
  ...
  try {
    ...
    const storagePath = path.join(
      storageRoot,
      `api-${config.testEnv}-${role}.storage.json`,
    );
    const state = await deps.tryReadState(storagePath);

    if (state && isStorageStateFresh(storagePath)) {
      logger.info("Storage state is fresh (another worker logged in)", {
        role,
        cacheKey,
      });
      return storagePath;
...
async function getStoredCookieWith(
  ...
  if (!state) {
    throw new Error(
      `Unable to read storage state for role "${role}" after ensure.`,
    );
  }
...
async function tryReadState(
  storagePath: string,
): Promise<{ cookies?: Array<{ name?: string }> } | undefined> {
  try {
