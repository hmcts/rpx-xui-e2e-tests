...
type SpecificAccessApprovalRequest = z.infer<
  typeof specificAccessApprovalSchema
>;

export function ensureCcdCaseReference(
  caseId: string,
  context: string,
): string {
...
export function buildCaseIdListPayload(caseId: string): { caseIds: string[] } {
  return {
    caseIds: [ensureCcdCaseReference(caseId, "role-access caseIds payload")],
  };
}

export function buildCaseIdPayload(caseId: string): { caseId: string } {
  return {
    caseId: ensureCcdCaseReference(caseId, "role-access caseId payload"),
  };
}
