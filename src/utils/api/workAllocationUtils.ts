...
export function assertLocationsListResponse(
  status: number,
  data: unknown,
): void {
  if (status !== 200) {
...
export function assertAvailableTasksResponse(
  status: number,
  data: unknown,
): void {
  if (status !== 200) {
...
export function assertMyWorkDashboardResponse(
  status: number,
  data: unknown,
): void {
  if (status === 200 && data) {
...
export function assertMyWorkTotalsResponse(
  status: number,
  data: unknown,
): void {
  if (status !== 200 || !data) {
...
export function assertCaseworkerListResponse(
  status: number,
  data: unknown,
): void {
...
export async function fetchTaskById(
  ...
  return apiClient.get(`workallocation/task/${id}`, {
    throwOnError: false,
  }) as Promise<TaskDetails>;
}
...
export function resolveSeededTaskIds(seeded?: SeededTaskResult): {
  sampleTaskId?: string;
  sampleMyTaskId?: string;
} {
  if (!seeded?.id) {
    return {};
  }
  return seeded.type === "assigned"
...
export async function runSeededAction(
  action: string,
  getId: () => string,
  deps: SeededActionDeps,
): Promise<boolean> {
  ...
  await withXsrfFn("solicitor", async (headers) => {
    const res = await deps.apiClient.post(
...
export function hasSeededEnvTasks(
  envTaskId?: string,
  envAssignedTaskId?: string,
): boolean {
