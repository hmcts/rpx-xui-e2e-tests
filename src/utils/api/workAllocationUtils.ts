...
type WorkAllocationApiClient = {
  post: (path: string, options?: Record<string, unknown>) => Promise<{ status: number; data?: unknown }>;
  get?: (path: string, options?: Record<string, unknown>) => Promise<{ status: number; data?: unknown }>;
};
