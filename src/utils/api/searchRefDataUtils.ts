...
export function assertGlobalSearchServices(
  status: number,
  data: unknown,
): void {
...
export function assertGlobalSearchResults(status: number, data: unknown): void {
  if (status === 200 && data) {
    ...
    if (
      ...
      const first = (data as { results: Array<{ caseReference?: unknown }> })
        .results[0];
...
export function assertSupportedJurisdictions(
  status: number,
  data: unknown,
): void {
  if (status === 200 && Array.isArray(data) && data.length > 0) {
...
export function assertStaffRefDataResponse(
  status: number,
  data: unknown,
): void {
...
export function assertMyAccessCount(status: number, data: unknown): void {
  if (status === 200) {
    if (typeof data === "number") {
      ...
      typeof (data as Partial<MyAccessCountResponse>)?.count === "number"
    ) {
...
export function assertRoleAccessGetResponse(
  status: number,
  data: unknown,
): void {
  if (status === 200) {
    if (Array.isArray(data) && data.length > 0) {
      ...
      Array.isArray(
        (data as RoleAssignmentContainer)?.roleAssignmentResponse,
      ) &&
      (data as RoleAssignmentContainer).roleAssignmentResponse!.length > 0
    ) {
      expectRoleAssignmentShape(
        (data as RoleAssignmentContainer).roleAssignmentResponse![0],
      );
    } else {
      expect(data).toEqual(expect.anything());
    }
...
export function assertRoleAccessByCaseIdResponse(
  status: number,
  data: unknown,
): void {
  ...
  if (
    status === 200 &&
    Array.isArray(typed?.roleAssignmentResponse) &&
    typed.roleAssignmentResponse.length > 0
  ) {
    expectRoleAssignmentShape(typed.roleAssignmentResponse[0]);
  }
}

export function assertRoleAssignmentsIfPresent(
  status: number,
  data: unknown,
): void {
  if (
...
export function assertManageLabellingResponse(
  status: number,
  data: unknown,
): void {
  if (status === 200 && Array.isArray(data) && data.length > 0) {
...
type Cookie = Partial<BrowserCookie> & Pick<BrowserCookie, "name" | "value">;

function normalizeCookie(cookie: Cookie): BrowserCookie {
  return {
    name: cookie.name,
    value: cookie.value,
...
export function buildExpiredCookies(state: unknown): BrowserCookie[] {
  const typed = state as Partial<StorageStateWithCookies>;
  return Array.isArray(typed.cookies)
    ? typed.cookies.map((cookie) => ({
