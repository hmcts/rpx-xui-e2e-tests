...
import type { AnnotationPayload, BookmarkPayload } from "./types";
...
export function resolveConfiguredDocId(
  explicit?: string,
  fallback?: string,
): string | undefined {
...
export function assertBinaryResponse(
  status: number,
...
export function resolveCreatedAnnotationId(
  data: unknown,
  fallback: string,
): string {
  const annotations = getAnnotations(data);
...
export function resolveCreatedBookmarkId(
  data: unknown,
  fallback: string,
): string {
...
export function resolveUserInfoId(data?: {
...
export async function resolveAnnotationSetId(
  ...
  try {
    const res = await apiClient.get(
      `em-anno/annotation-sets/filter?documentId=${docId}`,
      {
        headers,
        throwOnError: false,
      },
...
export async function buildBookmark(
  apiClient: ApiClient,
  docId: string,
...
export async function fetchUserId(
  ...
  return resolveUserInfoId(
    res.data as { userInfo?: { uid?: string; id?: string } },
  );
}
...
export async function uploadSyntheticDoc(
  deps: UploadDeps = {},
): Promise<string> {
  ...
  const requestFactory =
    deps.requestFactory ?? ((options) => playwrightRequest.newContext(options));
