...
export class GlobalSearchPage extends Base {
  ...
  readonly searchLinkOnMenuBar = this.page.locator(
    '.hmcts-primary-navigation__link[href="/search"]',
  );
  readonly searchButton = this.searchForm.getByRole("button", {
    name: "Search",
    exact: true,
  });
  readonly clearSearchButton = this.searchForm
  ...
  readonly caseDetailsLinks = this.page.locator(
    '.govuk-table a.govuk-link[href*="/cases/case-details/"]',
  );
  readonly viewLink = this.caseDetailsLinks.first();
  readonly changeSearchLink = this.page.locator('a[href*="/search"]');
  readonly searchResultsTable = this.page
    .locator("main")
    .getByRole("table")
    .first();
  readonly searchResultTable = this.searchResultsTable;
  readonly searchResultRows = this.searchResultsTable.locator("tbody tr");
  readonly applicantOrPartyName = this.page
    .locator(".govuk-form-group")
    .locator("#fullName");
  readonly paginationLinks = this.page.locator(
    ".govuk-width-container #pagination-label .hmcts-pagination__link",
  );
  readonly searchResultsHeader = this.page.locator(
    ".govuk-width-container .govuk-heading-xl",
  );
  readonly errorPageHeading = this.page.getByRole("heading", {
    name: /something went wrong/i,
  });

  async performGlobalSearchWithCase(
    caseId: string,
    caseType: string,
  ...
  async performGlobalSearchWithRetry(
    caseId: string,
    caseType: string,
    applicantOrPartyName?: string,
  ): Promise<void> {
    await this.performGlobalSearchWithCase(
      caseId,
      caseType,
      applicantOrPartyName,
    );
    const isErrorPage = await this.errorPageHeading
      .isVisible()
      .catch(() => false);
    if (!isErrorPage) {
      return;
    ...
    if (await this.errorPageHeading.isVisible().catch(() => false)) {
      throw new Error(
        'Global search returned "Something went wrong" after retry.',
      );
    }
  }
  ...
  async viewCaseDetails(caseId: string): Promise<void> {
    ...
    const caseLink = this.searchResultsTable
      .locator(
        `a.govuk-link[href*="/cases/case-details/"][href*="${normalizedCaseId}"]`,
      )
      .first();
  ...
  private async openCaseDetailsFromSearchResult(
    ...
    throw new Error(
      `Navigation to case details did not complete for ${caseId}. Current URL: ${this.page.url()}`,
    );
  }
  ...
  private async waitForSearchResults(caseId: string): Promise<void> {
    ...
    try {
      await matchingCaseLink.waitFor({
        state: "visible",
        timeout: EXUI_TIMEOUTS.SEARCH_SPINNER_RESULT_HIDDEN,
      });
    } catch {
