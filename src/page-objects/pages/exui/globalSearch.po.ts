...
export class GlobalSearchPage extends Base {
  ...
  async submitFromMenu(caseId: string, caseType: string, applicantOrPartyName?: string): Promise<void> {
    await expect(this.searchLinkOnMenuBar).toBeVisible({
  ...
  async performGlobalSearchWithCase(
    ...
    await this.submitFromMenu(caseId, caseType, applicantOrPartyName);
  ...
  private async waitForCaseDetailsUrl(caseId: string, timeoutMs: number): Promise<boolean> {
    try {
      await this.page.waitForURL(
        (url) => url.pathname.includes("/cases/case-details/") && url.pathname.includes(caseId),
        {
          timeout: timeoutMs
        }
      );
