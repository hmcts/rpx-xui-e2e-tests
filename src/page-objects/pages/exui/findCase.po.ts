...
export class FindCasePage extends Base {
  ...
  public async navigateToFindCase(): Promise<void> {
    for (let attempt = 1; attempt <= MAX_NAVIGATION_RETRY_ATTEMPTS; attempt++) {
      ...
      try {
        ...
        const shouldRetry =
          attempt < MAX_NAVIGATION_RETRY_ATTEMPTS && filterTimeout;
        if (!shouldRetry) {
          throw error;
        }
  ...
  private async openCaseDetailsFromSearchResult(
    initialSearchResultCaseLink: Locator,
    ...
    for (
      ...
      const searchResultCaseLink =
        attemptIndex === 0
          ? initialSearchResultCaseLink
          : this.searchResultsTable
              .locator(
                `a.govuk-link[href*="/cases/case-details/"][href*="${caseNumberFromUrl}"]`,
              )
              .first();
