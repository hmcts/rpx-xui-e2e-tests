...
export class FindCasePage extends Base {
  ...
  readonly showHideFilterButton = this.page
    .locator(".search-block button[aria-controls]")
    .first();
  readonly container = this.page.locator("exui-case-home");
  readonly filtersContainer = this.page.locator(
    ".search-block .hmcts-filter-layout__filter",
  );
  readonly applyFilterButton = this.page.locator(
    '.search-block button[type="submit"]',
  );
  readonly jurisdictionSelect = this.page
    .locator("#s-jurisdiction, #wb-jurisdiction, #cc-jurisdiction")
    .first();
  readonly caseTypeSelect = this.page
    .locator("#s-case-type, #wb-case-type, #cc-case-type")
    .first();
  readonly ccdCaseReference = this.page
    .locator(
      String.raw`#dynamicFilters #\[CASE_REFERENCE\], input[id*="CASE_REFERENCE"]`,
    )
    .first();
  readonly pagination = this.page.locator(".ngx-pagination");
  readonly searchResultsContainer = this.page.locator("#search-result");
  readonly searchResultsSummary = this.page.locator(
    "#search-result .pagination-top",
  );
  readonly searchResultsTable = this.page.locator(
    "ccd-search-result#search-result",
  );
  readonly searchResultsDataTable = this.searchResultsTable
    .locator("table")
    .first();
  readonly firstRowOfSearchResultsTable = this.searchResultsTable
    .locator(".govuk-link")
    .first();
  readonly workBasketFilterPanel = this.page
    .locator("ccd-search-filters-wrapper")
    .first();
  readonly findCaseLinkOnMenu = this.page
    .locator(
      '.hmcts-primary-navigation__nav .hmcts-primary-navigation__link[href*="case-search"]',
    )
    .first();
  readonly findCaseLinkOnTopRight = this.page
    .locator(
      '.hmcts-primary-navigation__search .hmcts-primary-navigation__link[href*="case-search"]',
    )
    .first();
    ...
    for (let attempt = 1; attempt <= MAX_NAVIGATION_RETRY_ATTEMPTS; attempt++) {
      ...
      try {
        ...
        const jurisdictionBootstrapFailed =
          this.hasJurisdictionBootstrapFailure();
        const filterTimeout = this.isFindCaseFilterTimeout(error);
        const shouldRetry =
          attempt < MAX_NAVIGATION_RETRY_ATTEMPTS &&
          (jurisdictionBootstrapFailed || filterTimeout);
        if (!shouldRetry) {
          throw error;
        }

        this.logger.warn(
          "Find case filter bootstrap failed; retrying case search page",
          {
            attempt,
  ...
  /**
   * Waits for Find case page scaffolding/controls to become ready after navigation.
   *
   * Some environments render the heading late or not at all while controls are already usable.
   * This readiness check uses stable form controls instead of relying on `h1` text.
   */
  public async waitForFindCasePageReady(): Promise<void> {
    await this.page.waitForURL(/\/cases\/case-search/, {
   ...
   */
  public async fillSearchCriteria(
    caseNumber: string,
    caseType: string,
    jurisdiction: string,
  ): Promise<void> {
  ...
  public async displayCaseDetailsFor(caseNumber: string): Promise<void> {
    ...
    if (normalizedCaseNumber.length !== CCD_CASE_REFERENCE_LENGTH) {
      throw new Error(
        `Expected ${CCD_CASE_REFERENCE_LENGTH}-digit case reference, received "${caseNumber}"`,
      );
    }

    await this.searchResultsDataTable.waitFor({
      state: "visible",
    ...
    const caseDetailsLink = this.searchResultsTable
      .locator(
        `a.govuk-link[href*="/cases/case-details/"][href*="${normalizedCaseNumber}"]`,
      )
      .first();
    await caseDetailsLink.waitFor({
      state: "visible",
      timeout: EXUI_TIMEOUTS.SEARCH_SPINNER_RESULT_HIDDEN,
    });
    await this.openCaseDetailsFromSearchResult(
      caseDetailsLink,
  ...
  private async ensureFiltersVisible() {
    const needsOpenFilterPanel = !(await this.jurisdictionSelect
      .isVisible()
      .catch(() => false));
    if (
      needsOpenFilterPanel &&
      (await this.showHideFilterButton.isVisible().catch(() => false))
    ) {
    ...
    await this.caseTypeSelect.waitFor({
      state: "visible",
      timeout: EXUI_TIMEOUTS.SEARCH_FIELD_VISIBLE,
    });
    await this.ccdCaseReference.waitFor({
      state: "visible",
      timeout: EXUI_TIMEOUTS.SEARCH_FIELD_VISIBLE,
    });
    await this.exuiCaseListComponent.filters.applyFilterBtn.waitFor({
      state: "visible",
    });
  }
  ...
  private isFindCaseFilterTimeout(error: unknown): boolean {
    const message =
      error instanceof Error ? error.message : (JSON.stringify(error) ?? "");
    return /locator\('#s-jurisdiction, #wb-jurisdiction, #cc-jurisdiction'\).*to be visible/i.test(
      message,
    );
  }
  ...
  private async openFindCaseVia(link: Locator): Promise<void> {
    ...
    if (linkVisible) {
      ...
      this.logger.info(
        "Find case navigation link not visible; falling back to direct case-search URL",
        {
    ...
    await this.waitForFindCasePageReady();
  }
  ...
  private async waitForCaseDetailsUrl(
    caseNumber: string,
    timeoutMs: number,
  ): Promise<boolean> {
    try {
