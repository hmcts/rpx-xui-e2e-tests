...
export class CaseListPage extends Base {
  ...
  readonly resetFilterButton = this.page.locator(
    '.search-block button[type="reset"]',
  );
  ...
  readonly textField0FallbackInput = this.page
    .locator(
      'input[id*="TextField0"], input[name*="TextField0"], input[formcontrolname*="TextField0"]',
    )
    .first();
  readonly quickSearchContainer = this.page.locator(
    ".hmcts-primary-navigation__global-search",
  );
  readonly quickSearchCaseReferenceInput = this.page.locator(
    "#exuiCaseReferenceSearch",
  );
  readonly quickSearchFindButton = this.quickSearchContainer.getByRole(
    "button",
    { name: "Find", exact: true },
  );
  ...
  public async searchByTextField0(textField0: string): Promise<boolean> {
    ...
    if (!(await input.isVisible().catch(() => false))) {
      const canToggleFilters = await this.showHideFilterButton
        .isVisible()
        .catch(() => false);
      if (canToggleFilters) {
  ...
  public async quickSearchByCaseReference(
    caseReference: string,
  ): Promise<boolean> {
    const inputVisible = await this.quickSearchCaseReferenceInput
      .isVisible()
      .catch(() => false);
    const buttonVisible = await this.quickSearchFindButton
      .isVisible()
      .catch(() => false);
