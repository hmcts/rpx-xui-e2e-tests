...
export function extractCaseNumberFromUrl(url: string): string | null {
  try {
    ...
    const detailsPathMatch = /\/cases\/case-details\/(\d{16})(?:$|\/)/.exec(
      pathname,
    );
...
export class CaseDetailsPage extends Base {
  ...
  readonly continueButton = this.container.getByRole("button", {
    name: "Continue",
    exact: true,
  });
  ...
  readonly caseFlagApplicantFlagTable = this.page.locator(
    "table.govuk-table.ng-star-inserted",
  );
  ...
  readonly caseViewerTable = this.page.getByRole("table", {
  ...
  readonly caseProgressMessage = this.page.locator(
    "#progress_legalOfficer_updateTrib_dismissed_under_rule_31",
  );
  readonly resultsNotFoundHeading = this.page
    .locator("exui-no-results")
    .getByRole("heading", { level: 1 });
  readonly backLink = this.page.locator(
    "exui-no-results .govuk-width-container .govuk-back-link",
  );

  // GlobalSearch
  readonly tabsCount = this.page.locator(
    ".mat-tab-label-container .mat-tab-list",
  );
  readonly caseSummaryHeading = this.page.locator(
    "#case-viewer-field-read--caseSummaryTabHeading",
  );
  readonly addOrRemoveFlagsLink = this.page.locator("#addCaseFlagEventLink");
  readonly extend26WeekTimelineLink = this.page.locator(
    "#extend26WeekTimelineLink",
  );
  //Find Search
   ...
   */
  async getCaseDetailsTabData(
    tabClass: string,
  ): Promise<Record<string, string>> {
    const tableLocator = this.page
      .locator(".case-viewer-container")
      .locator(`table.${tabClass}`);
    try {
      await tableLocator.waitFor({
        state: "visible",
        timeout: TIMEOUTS.TABLE_VISIBLE,
      });
      return this.parseKeyValueTable(tableLocator);
    } catch (error) {
      // Primary table not found, fall back to generic tabpanel table
      this.logger.error(
        `Table with class '${tabClass}' not found, using fallback selector`,
        error,
      );
    ...
    for (let i = 0; i < tables.length; i++) {
      const hasHeaders = await tables[i].evaluate((table: Element) => {
        ...
        const headerCells = new Set(
          Array.from(thead.querySelectorAll("th, td")).map((el) =>
            (el.textContent || "").trim(),
          ),
        );
        return (
  ...
  async getCaseNumberFromAlert(): Promise<string> {
    await this.caseAlertSuccessMessage.waitFor({
      state: "visible",
      timeout: TIMEOUTS.ALERT_VISIBLE,
    });
    ...
    if (!caseNumberMatch) {
      this.logger.error("Failed to extract case number from alert", {
        alertLength: alertText?.length,
      });
  ...
  async getCaseViewerRowByName(rowName: string): Promise<Locator> {
    await this.caseViewerTable.waitFor({
      state: "visible",
      timeout: TIMEOUTS.TABLE_VISIBLE,
    });
    ...
    throw new Error(
      `Unable to find case viewer row with first-cell label "${rowName}"`,
    );
  }
  ...
  async selectCaseAction(
    ...
    try {
      ...
      this.logger.warn(
        "Failed to select option by label, falling back to value selector",
        { error },
      );
    ...
    try {
      ...
      this.logger.warn(
        "Expected locator not visible after case action; retrying action",
        { action },
      );
      try {
        await this.caseActionsDropdown.selectOption({ label: action });
        ...
        this.logger.warn(
          "Retry: failed to select option by label, falling back to value selector",
          { retryError },
        );
        await this.caseActionsDropdown.selectOption(action);
      }
  ...
  async selectPartyFlagTarget(target: string, flagType: string) {
    ...
    try {
      ...
      this.logger.warn("Exact label not found, using regex fallback", {
        error,
      });
    ...
    await this.commonRadioButtons
      .getByLabel(flagType)
      .waitFor({ state: "visible", timeout: this.getRecommendedTimeoutMs() });
  ...
  async hasCallbackValidationErrorAlert(timeoutMs = 1000): Promise<boolean> {
    const callbackValidationAlert = this.page
      ...
      .first();
    return callbackValidationAlert
  ...
  async selectCaseDetailsTab(tabName: string) {
    ...
    const escapedTabName = tabName.replaceAll(
      /[.*+?^${}()|[\]\\]/g,
      String.raw`\$&`,
    );
    const tab = this.page
      .getByRole("tab", { name: new RegExp(escapedTabName, "i") })
      .first();
    await tab.waitFor({ state: "visible", timeout: tabLoadTimeoutMs });
    await tab.click();
    await this.waitForSpinnerToComplete(
      `after selecting "${tabName}" tab`,
      tabLoadTimeoutMs,
    ).catch(() => {
    ...
    const visibleTabPanel = this.page
  ...
  private async waitForTabPanelReadiness(
    tabPanel: Locator,
    timeoutMs: number,
  ): Promise<void> {
    await tabPanel.waitFor({ state: "visible", timeout: timeoutMs });
    const structuredContent = tabPanel.locator(
      "table, form, ccd-read-collection-field, ccd-read-complex-type-field",
    );
