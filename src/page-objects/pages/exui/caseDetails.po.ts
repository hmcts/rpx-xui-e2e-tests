...
import { EXUI_TIMEOUTS as TIMEOUTS } from "./exui-timeouts.js";
...
export class CaseDetailsPage extends Base {
  ...
  readonly caseHeader = this.page.locator('ccd-case-header').first();
  readonly caseViewerShell = this.page.locator('.case-viewer-container').first();
  ...
  private async waitForCaseDetailsPoll(intervalMs: number): Promise<void> {
    // Some case-details shell elements appear without a navigation or network signal to await.
  ...
  private async _runOnRows<T>(selector: string | Locator, fn: (rows: Element[]) => T): Promise<T> {
    ...
    // Selector string: resolve through a Locator so the table traversal stays within Playwright's locator model.
    return this.page.locator(selector).locator('tr').evaluateAll(fn);
  }
  ...
  async selectCaseAction(
    ...
    await this.waitForSpinnerToComplete('after selecting case action', timeoutMs);
  ...
  async openRolesAndAccessTab(caseId: string): Promise<void> {
    ...
    await this.acceptAnalyticsCookies().catch(() => undefined);
    await this.waitForCaseDetailsTabsReady(this.getRecommendedTimeoutMs());
  ...
  async openDocumentOneInMediaViewer(): Promise<Page> {
    ...
    const currentPagePromise = this.page
      .waitForFunction(
        (routePatternSource) => {
    ...
    const resolvedPage = await Promise.race([
      popupPromise.then((page) => ({ kind: "popup" as const, page })),
      currentPagePromise.then((page) => ({ kind: "current" as const, page }))
    ]);

    if (resolvedPage.page) {
      if (resolvedPage.kind === "popup") {
        await resolvedPage.page.waitForLoadState("domcontentloaded").catch(() => undefined);
        await resolvedPage.page.waitForURL(MEDIA_VIEWER_ROUTE_PATTERN, { timeout: navigationTimeoutMs }).catch(() => undefined);
      }
  ...
  private async waitForCaseDetailsTabsReady(timeoutMs: number): Promise<void> {
    ...
    while (Date.now() < deadline) {
      ...
      const [shellVisible, tabListVisible] = await Promise.all([
        this.isCaseDetailsShellVisible(),
        this.tabList.isVisible().catch(() => false)
      ]);

      if (!shellVisible || !tabListVisible) {
  ...
  private async isCaseDetailsShellVisible(): Promise<boolean> {
    const [containerVisible, caseHeaderVisible, caseViewerShellVisible, dropdownVisible, buttonVisible] = await Promise.all([
      this.container.isVisible().catch(() => false),
      this.caseHeader.isVisible().catch(() => false),
      this.caseViewerShell.isVisible().catch(() => false),
      this.caseActionsDropdown.isVisible().catch(() => false),
      this.caseActionGoButton.isVisible().catch(() => false),
    ]);

    return containerVisible || caseHeaderVisible || caseViewerShellVisible || (dropdownVisible && buttonVisible);
  }
