...
export class TaskListPage extends Base {
  readonly myWorkHeading = this.page
    .getByRole("heading", { name: /my work/i })
    .first();
  readonly taskListFilterToggle = this.page
    .locator("exui-task-list-filter .govuk-button.hmcts-button--secondary")
    .first();
  readonly filterPanel = this.page.locator("xuilib-generic-filter");
  readonly selectAllServicesFilter = this.filterPanel
    .locator("input#checkbox_servicesservices_all")
    .first();
  ...
  readonly selectServicesError = this.filterPanel
    .locator("#services-error")
    .first();
  readonly typesOfWorkCheckBoxes = this.filterPanel.locator(
    "#types-of-work #checkbox_types-of-work .govuk-checkboxes__item",
  );
  readonly selectAllTypesOfWorksFilter = this.filterPanel
    .locator("input#checkbox_types-of-worktypes_of_work_all")
    .first();
  ...
  readonly selectTypesOfWorksError = this.filterPanel
    .locator("#types-of-work-error")
    .first();
  readonly applyFilterButton = this.page.locator("button#applyFilter").first();

  readonly taskTableTabs = this.page.locator(
    ".hmcts-sub-navigation .hmcts-sub-navigation__link",
  );

  readonly taskListTable = this.page.locator("table.govuk-table").first();
  readonly sortByCaseNameTableHeader =
    this.taskListTable.locator("#sort_by_caseName");
  readonly sortByCaseCategoryTableHeader = this.taskListTable.locator(
    "#sort_by_caseCategory",
  );
  readonly sortByLocationTableHeader = this.taskListTable.locator(
    "#sort_by_locationName",
  );
  readonly sortByTaskTableHeader =
    this.taskListTable.locator("#sort_by_taskTitle");
  readonly sortByDueDateTableHeader =
    this.taskListTable.locator("#sort_by_dueDate");
  readonly sortByHearingDateTableHeader = this.taskListTable.locator(
    "#sort_by_next_hearing_date",
  );
  ...
  readonly taskActionMarkAsDone =
    this.taskActionsRow.locator("#action_complete");
  ...
  readonly taskActionClaimAndGo = this.taskActionsRow.locator(
    "#action_claim-and-go",
  );
  readonly confirmCancelTaskButton = this.page.getByRole("button", {
    name: "Cancel task",
  });
  readonly caseDetailsTaskActionCancel = this.page
    .locator("#action_cancel")
    .or(this.page.getByRole("link", { name: "Cancel task" }));
  readonly cancelledTaskMessage = this.page.getByText(
    "You've cancelled a task. It has been removed from the task list.",
  );
  readonly taskNoLongerAvailableMessage = this.page.getByText(
    "The task is no longer available.",
  );

  readonly paginationControls = this.page.locator(".ngx-pagination");
  readonly paginationNextButton =
    this.paginationControls.locator(".pagination-next");
  readonly paginationEllipsisButton =
    this.paginationControls.locator(".ellipsis");
  readonly paginationPreviousButton = this.paginationControls.locator(
    ".pagination-previous",
  );
  ...
  readonly reassignUserAutocompleteOverlay =
    this.page.locator(".cdk-overlay-pane");
  readonly reassignUserAutocompleteFirstOption = this.page
    .getByRole("option")
    .first();
  ...
  private async waitForTaskListSpinnerToSettle(
    timeoutMs: number,
  ): Promise<void> {
    await this.page
      .waitForFunction(
        () => document.querySelectorAll("xuilib-loading-spinner").length === 0,
        undefined,
        { timeout: timeoutMs },
      )
      .catch(() => undefined);
  ...
  async waitForTaskListShellReady(context: string) {
    await this.page
      .waitForURL(/\/(?:work\/my-work\/(?:list|available)|service-down)/, {
    ...
    const bootstrapSignal = await Promise.any([
      ...
      this.page
        ...
        .filter({ hasText: /my work/i })
        .first()
        .waitFor({ state: "visible", timeout: TASK_LIST_READY_TIMEOUT_MS })
        .then(() => "generic-my-work-heading"),
      this.taskTableTabs
        .first()
        .waitFor({ state: "visible", timeout: TASK_LIST_READY_TIMEOUT_MS })
        .then(() => "tabs"),
      this.taskListFilterToggle
        .waitFor({ state: "visible", timeout: TASK_LIST_READY_TIMEOUT_MS })
        .then(() => "filter-toggle"),
      this.taskListTable
        .waitFor({ state: "visible", timeout: TASK_LIST_READY_TIMEOUT_MS })
      ...
      this.taskTableHeader
        .waitFor({ state: "visible", timeout: TASK_LIST_READY_TIMEOUT_MS })
        .then(() => "table-header"),
      this.taskTableFooter
        .waitFor({ state: "visible", timeout: TASK_LIST_READY_TIMEOUT_MS })
        .then(() => "table-footer"),
      this.taskListResultsAmount
        .waitFor({ state: "visible", timeout: TASK_LIST_READY_TIMEOUT_MS })
        .then(() => "results-summary"),
      this.errorPageHeading
        .waitFor({ state: "visible", timeout: TASK_LIST_READY_TIMEOUT_MS })
        .then(() => "error-page"),
      this.serviceDownError
        .waitFor({ state: "visible", timeout: TASK_LIST_READY_TIMEOUT_MS })
        .then(() => "service-down"),
    ]).catch(async () => {
    ...
    if (bootstrapSignal === "error-page") {
      await this.assertTaskListHealthy(
        `waiting for task list shell (${context})`,
      );
    }
  ...
  private resolveInteractionTimeout(
  ...
  private async waitForFilterControls(
    context: string,
    deadlineMs?: number,
  ): Promise<void> {
    const timeoutMs = this.resolveInteractionTimeout(
      deadlineMs,
      FILTER_CONTROL_READY_TIMEOUT_MS,
    );
    await Promise.any([
      this.applyFilterButton.waitFor({ state: "visible", timeout: timeoutMs }),
      this.taskListFilterToggle.waitFor({
        state: "visible",
        timeout: timeoutMs,
      }),
    ]).catch(async () => {
      ...
      throw new Error(
        `Task list filter controls did not become visible within ${timeoutMs}ms.`,
      );
    });
  }

  private async waitForFilterCheckboxVisible(
    checkbox: Locator,
    description: string,
    deadlineMs?: number,
  ): Promise<Locator> {
  ...
  private assertFilterInteractionAlive(
    context: string,
    deadlineMs?: number,
  ): void {
    ...
    if (deadlineMs && Date.now() > deadlineMs) {
      throw new Error(
        `Task list filter ${context} did not complete within ${FILTER_GROUP_OPERATION_TIMEOUT_MS}ms.`,
      );
    }
  }

  private async readFilterCheckboxState(
    checkbox: Locator,
    description: string,
    deadlineMs?: number,
  ): Promise<boolean> {
    this.assertFilterInteractionAlive(
      `checkbox "${description}" state read`,
      deadlineMs,
    );
  ...
  async openFilterPanel(deadlineMs?: number) {
    ...
    const panelDeadlineMs =
      deadlineMs ?? Date.now() + FILTER_PANEL_READY_TIMEOUT_MS;
    while (Date.now() < panelDeadlineMs) {
      ...
      await this.filterPanel
        .waitFor({
          state: "visible",
          timeout: this.resolveInteractionTimeout(panelDeadlineMs, 1_000),
        })
        .catch(() => undefined);
      if (await this.applyFilterButton.isVisible().catch(() => false)) {
        return;
    ...
    throw new Error(
      `Task list filter panel did not become ready within ${
        deadlineMs
          ? FILTER_GROUP_OPERATION_TIMEOUT_MS
          : FILTER_PANEL_READY_TIMEOUT_MS
      }ms.`,
    );
  }
  ...
  async applyCurrentFilters() {
    await this.openFilterPanel();
    await this.applyFilterButton.evaluate((button: HTMLButtonElement) =>
      button.click(),
    );
  }

  async setSelectAllServicesFilter(checked: boolean) {
    await this.setFilterCheckbox(
      this.selectAllServicesFilter,
      checked,
      "select all services",
    );
  }
  ...
  async setSelectAllTypesOfWorksFilter(checked: boolean) {
    await this.setFilterCheckbox(
      this.selectAllTypesOfWorksFilter,
      checked,
      "select all types of work",
    );
  }

  async clearServicesFilters() {
    await this.clearFilterGroup(
      "services",
      this.selectAllServicesFilter,
  ...
  private async setFilterCheckbox(
    checkbox: Locator,
    checked: boolean,
    description: string,
    deadlineMs?: number,
    ...
    const targetCheckbox = await this.waitForFilterCheckboxVisible(
      checkbox,
      description,
      deadlineMs,
    );
    for (let attempt = 0; attempt < FILTER_INTERACTION_ATTEMPTS; attempt += 1) {
      const isChecked = await this.readFilterCheckboxState(
        targetCheckbox,
        description,
        deadlineMs,
      );
      ...
      await this.page.waitForTimeout(250);
      if (
        (await this.readFilterCheckboxState(
          targetCheckbox,
          description,
          deadlineMs,
        )) === checked
  ...
  private async clearFilterGroup(
    groupName: string,
    ...
    await this.forceChildCheckboxState(
      childCheckboxes,
      false,
      groupName,
      deadlineMs,
    ...
    if (
      await this.groupCheckboxesMatchState(
        childCheckboxes,
        false,
        groupName,
        deadlineMs,
  ...
  private async forceChildCheckboxState(
    childCheckboxes: Locator,
    checked: boolean,
    groupName: string,
    deadlineMs?: number,
    ...
    for (let index = 0; index < checkboxCount; index += 1) {
      this.assertFilterInteractionAlive(
        `${groupName} child checkbox update`,
        deadlineMs,
      );
  ...
  private async getCheckboxStates(
    childCheckboxes: Locator,
    groupName: string,
    deadlineMs?: number,
  ) {
    ...
    for (let index = 0; index < checkboxCount; index += 1) {
      this.assertFilterInteractionAlive(
        `${groupName} checkbox state collection`,
        deadlineMs,
      );
      states.push(
        await this.readFilterCheckboxState(
          childCheckboxes.nth(index),
          `${groupName} option ${index + 1}`,
          deadlineMs,
        ),
      );
  ...
  async waitForManageButton(
    ...
    while (Date.now() < deadline) {
      await this.assertTaskListInteractive(
        `waiting for Manage button (${context})`,
      );
  ...
  async openFirstManageActions(
    ...
    while (Date.now() < deadline) {
      ...
      const expanded = await this.taskActionsRow
        .waitFor({
          state: "visible",
          timeout: Math.max(1_000, Math.min(2_500, deadline - Date.now())),
        })
        .then(() => true)
        .catch(() => false);
      ...
      await this.assertTaskListInteractive(
        `opening Manage actions (${context})`,
      );
      attempt += 1;
    ...
    throw new Error(
      `Timed out after ${timeoutMs}ms opening Manage actions (${context}). url=${this.page.url()}`,
    );
  }
  ...
  async clickTaskAction(
    ...
    while (Date.now() < deadline) {
      ...
      const actionTimeoutMs = Math.max(
        1_000,
        Math.min(2_500, deadline - Date.now()),
      );
      const clickStrategies = [
        ...
        async () =>
          targetAction.evaluate((actionElement: HTMLAnchorElement) =>
            actionElement.click(),
          ),
        async () => {
    ...
    throw new Error(
      `Timed out after ${timeoutMs}ms clicking task action (${context}). url=${this.page.url()}`,
    );
  }
  ...
  async clickButtonAndWaitForRequest(
    ...
    await targetButton.waitFor({
      state: "visible",
      timeout: Math.max(1_000, Math.min(5_000, timeoutMs)),
    });
    await targetButton
      .scrollIntoViewIfNeeded({
        timeout: Math.max(250, Math.min(1_500, timeoutMs)),
      })
      .catch(() => undefined);
    ...
    for (let attempt = 0; attempt < 5 && Date.now() < deadline; attempt += 1) {
      ...
      try {
        if (attempt === 0) {
          ...
          await targetButton.evaluate((buttonElement: HTMLButtonElement) =>
            buttonElement.click(),
          );
        } else {
