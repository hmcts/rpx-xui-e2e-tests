...
export class TaskListPage extends Base {
  ...
  private async waitForTaskListPoll(intervalMs: number): Promise<void> {
    // The task list filter panel redraws outside the normal request lifecycle, so short polling is intentional.
  ...
  private async clickWithForceFallback(
    ...
    try {
      ...
      if (!/intercepts pointer events|element is not stable|element is not visible/i.test(message)) {
        throw error;
      }
  ...
  async setServiceFilterByLabel(label: string, checked: boolean) {
    await this.setFilterCheckbox(this.filterPanel.getByLabel(label), checked, `service ${label}`);
  }

  async setTypeOfWorkFilterByLabel(label: string, checked: boolean) {
    await this.setFilterCheckbox(this.filterPanel.getByLabel(label), checked, `type of work ${label}`);
  }

  async getFirstServiceFilterValue(): Promise<string | null> {
    await this.openFilterPanel();
    return this.serviceFilterCheckboxes.first().getAttribute('value');
  }

  async getFirstTypeOfWorkFilterValue(): Promise<string | null> {
    await this.openFilterPanel();
    return this.typesOfWorkFilterCheckboxes.first().getAttribute('value');
  }

  async setFirstServiceFilterChecked(checked: boolean) {
    await this.setFilterCheckbox(this.serviceFilterCheckboxes.first(), checked, 'first service filter');
  }

  async setFirstTypeOfWorkFilterChecked(checked: boolean) {
    await this.setFilterCheckbox(this.typesOfWorkFilterCheckboxes.first(), checked, 'first type of work filter');
  }
  ...
  private async isFilterPanelOpen(): Promise<boolean> {
    return this.applyFilterButton.isVisible().catch(() => false);
  }
  ...
  async searchForLocation(searchText: string) {
    ...
    await this.allWorkLocationSearchInput.dispatchEvent('input');
  }
  ...
  async selectAllWorkAutocompleteOption(optionText: string) {
    const option = this.autocompleteOverlay
      .last()
  ...
  async waitForTaskRowReady(
    ...
    while (Date.now() < deadline) {
      ...
      await this.waitForTaskListPoll(pollMs);
    }
  ...
  private async openManageActions(
    ...
    while (Date.now() < deadline) {
      ...
      attempt += 1;
      await this.waitForTaskListPoll(Math.min(pollMs, Math.max(0, deadline - Date.now())));
    }
