...
export class CreateCasePage extends Base {
  ...
  readonly somethingWentWrongHeading = this.page.getByRole("heading", {
  ...
  private async waitForSelectReady(selector: string, timeoutMs?: number) {
    ...
    await this.page.waitForFunction(
      (sel) => {
        ...
        const hasRealOption = Array.from(el.options).some((option) => {
          const normalizedLabel = option.label.trim().toLowerCase();
          const normalizedValue = option.value.trim().toLowerCase();
          const isPlaceholder =
            normalizedValue === "" ||
            normalizedLabel.includes("select a value");
          return !isPlaceholder;
        });
        return (
          hasRealOption ||
          (el.options.length > 0 &&
            Array.from(el.options).some((option) => option.value.trim() !== ""))
        );
      },
      selector,
  ...
  private async clickContinueAndWait(
    ...
    const settleDeadline = Date.now() + stepTimeout;
    let visibleContinueButton: Locator | null = null;
    while (Date.now() < settleDeadline) {
      visibleContinueButton = null;
      const liveContinueButton = await this.getVisibleActionButton(
        this.continueButton,
      );
      if (liveContinueButton) {
        visibleContinueButton = liveContinueButton;
        await liveContinueButton.scrollIntoViewIfNeeded().catch(() => {
          // Re-render race: continue loop and reacquire fresh locator.
        });
        const enabled = await liveContinueButton.isEnabled().catch(() => false);
        if (enabled) {
          break;
        }
      }
      await this.waitForSpinnerToComplete(
        `while waiting for continue button ${context}`,
        1_000,
      ).catch(() => {
        // Best effort while backend APIs settle.
      });
    ...
    await expect(visibleContinueButton).toBeEnabled({
      timeout: Math.max(1_000, Math.min(stepTimeout, 15_000)),
    });
  ...
  async clickSubmitAndWait(
    ...
    const autoAdvanceTimeoutMs = Math.max(
      ...
      Math.min(
        EXUI_TIMEOUTS.SUBMIT_AUTO_ADVANCE_MAX,
    ...
    while (Date.now() < deadline) {
      if (this.page.isClosed()) {
        throw new Error(
          `Page closed while waiting for submit button ${context}`,
        );
      }
      ...
      const visibleSubmitButton = await this.getVisibleActionButton(
        this.submitButton,
      );
      if (visibleSubmitButton) {
      ...
      if (spinnerVisible) {
        ...
        await this.page.waitForTimeout(
          EXUI_TIMEOUTS.SUBMIT_SPINNER_STABILIZE_WAIT,
        );
        continue;
      }
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        if (jurisdictionBootstrapFailed || onSomethingWentWrongPage) {
          ...
          await this.page.waitForTimeout(
            EXUI_TIMEOUTS.CREATE_CASE_RETRY_BACKOFF,
          );
        } else {
  ...
  async createCaseEmployment(jurisdiction: string, caseType: string) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        let startEventSelected = false;
        let lastStartEventError: unknown;
        let selectedEventType:
          | (typeof createCaseEventOptions)[number]
          | undefined;
        for (const eventType of createCaseEventOptions) {
          try {
            await this.createCase(jurisdiction, caseType, eventType);
            startEventSelected = true;
            selectedEventType = eventType;
            break;
          } catch (error) {
            lastStartEventError = error;
            const message = this.normalizeUnknownError(error);
            const isMissingCreateCaseEvent =
              eventType === "Create Case" &&
              message.includes('Option not found for "Create Case"');
            if (isMissingCreateCaseEvent) {
              logger.warn(
                'Employment "Create Case" event is unavailable; falling back to "Create draft claim".',
                {
                  jurisdiction,
                  caseType,
                },
        ...
        if (!startEventSelected) {
          throw (
            lastStartEventError ??
            new Error("Unable to select an employment case creation event.")
          );
        }
        if (selectedEventType === "Create draft claim") {
          throw new Error(
