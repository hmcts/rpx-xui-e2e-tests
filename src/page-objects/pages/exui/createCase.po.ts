...
import { clickSubmitAndWaitFlow, findCreateCaseBootstrapFailure, startCreateCaseFlow } from './createCase.flow';
...
export class CreateCasePage extends Base {
  ...
  async clickContinueAndWait(
    ...
    await expect(visibleContinueButton).toBeEnabled({ timeout: enabledTimeout });
  ...
  async clickSubmitAndWait(context: string, options: { timeoutMs?: number; maxAutoAdvanceAttempts?: number } = {}) {
    ...
    await clickSubmitAndWaitFlow({
      ...
      getVisibleActionButton: async (locator) => (await this.getVisibleActionButton(locator)) ?? undefined,
  ...
  async uploadFile(fileName: string, mimeType: string, fileContent: string, fileInput?: Locator) {
    ...
    const visibleFileInput = this.page.locator('input[type="file"]:visible').first();
    ...
    await this.fileUploadStatusLabel.waitFor({ state: 'hidden', timeout: EXUI_TIMEOUTS.UPLOAD_RESPONSE }).catch(async () => {
      const uploadStatusText = (await this.fileUploadStatusLabel.first().textContent().catch(() => ''))?.trim();
      if (uploadStatusText) {
        throw new Error(`Upload did not complete cleanly: ${uploadStatusText}`);
      }
    });
  }
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        if (isExternalDraftClaimFlow) {
          ...
          await this.clickSubmitAndWait('after external draft claim work location', {
            timeoutMs: 60_000,
            maxAutoAdvanceAttempts: 1,
          });
        ...
        if (!(await this.respondentOrganisation.isChecked().catch(() => false))) {
          try {
        ...
        if (!(await this.claimantRepresentedNo.isVisible().catch(() => false))) {
  ...
  private async prepareEmploymentDraftUploadPage(): Promise<void> {
    ...
    for (let attempt = 1; attempt <= maxAdvanceAttempts; attempt += 1) {
      ...
      if (!(await visibleContinueButton.isVisible().catch(() => false))) {
        throw new Error(`Employment draft update did not reach a document upload page; no Continue button visible at ${this.page.url()}`);
      }
      await this.clickContinueAndWait(`employment draft upload preparation step ${attempt}`);
    }

    throw new Error('Employment draft update did not reach the document upload page after 8 steps');
  }
  ...
  private async reEnterEmploymentDraftUpdateFromCaseDetails(): Promise<boolean> {
    ...
    try {
      ...
      const updateCaseOption = await caseActionsDropdown.evaluate((element) => {
        ...
        const option = Array.from(element.options).find((candidate) => /update case/i.test(candidate.label));
    ...
    await this.waitForSpinnerToComplete('after re-entering employment update-case flow');
    await this.page.waitForLoadState('domcontentloaded');
    await this.assertNoEventCreationError('after re-entering employment update-case flow');
    return true;
  }
  ...
  async selectDivorceReasons(reasons: string[]) {
    ...
    for (const reason of reasons) {
      ...
      const checkboxVisible = await divorceReasonCheckbox.isVisible().catch(() => false);
      if (checkboxVisible) {
        await divorceReasonCheckbox.check({ force: true });
        continue;
      }

      const checkboxId = await divorceReasonCheckbox.getAttribute('id').catch(() => null);
      const labelLocator =
  ...
  async fillDivorcePocSections(
    ...
    const fillOnce = async () => {
      const fillPerson = async (person: 'person1' | 'person2', data?: Partial<PersonData>) => {
        if (!data) {
          return;
        }
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        logger.warn('Divorce PoC section fill failed during wizard advance; retrying from start page', {
          attempt,
          maxAttempts,
        });
        await this.page.goto(startUrl);
        await this.person1TitleInput.waitFor({ state: 'visible', timeout: EXUI_TIMEOUTS.POC_FIELD_VISIBLE });
      }
    }
  }
