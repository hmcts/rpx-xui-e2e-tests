...
export class CaseSearchPage extends Base {
  ...
  readonly applyButton = this.page.locator(
    'button[title="Apply filter"], button[aria-label="Apply filter"]',
  );
  ...
  private async selectOptionByLabel(
    ...
    const match = options.find((option) =>
      option.toLowerCase().includes(target),
    );
    if (!match) {
      throw new Error(
        `Search filter option "${label}" not found. Available: ${options.join(", ")}`,
      );
    }
