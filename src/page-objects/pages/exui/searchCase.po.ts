...
export class SearchCasePage extends Base {
  ...
  readonly quickSearchContainerFallback = this.page
    .locator("li:has(#exuiCaseReferenceSearch)")
    .first();
  readonly caseIdTextBox = this.page.locator("#exuiCaseReferenceSearch");
  readonly searchCaseFindButton = this.quickSearchContainer.getByRole(
    "button",
    { name: "Find", exact: true },
  );
  readonly searchCaseFindButtonFallback =
    this.quickSearchContainerFallback.getByRole("button", {
      name: "Find",
      exact: true,
    });
  readonly caseProgressHeading = this.page
    .locator("#progress_legalOfficer_updateTrib_dismissed_under_rule_31 h2")
    .first();
  readonly caseProgressPanel = this.page.locator(
    "#progress_legalOfficer_updateTrib_dismissed_under_rule_31",
  );
  readonly noResultsHeading = this.page.locator(
    "exui-no-results .govuk-heading-xl",
  );
  ...
  private async waitForPostSearchSpinnerCycle(): Promise<void> {
    ...
    try {
      await spinner.waitFor({
        state: "visible",
        timeout: EXUI_TIMEOUTS.SPINNER_APPEAR_BRIEF,
      });
      await spinner.waitFor({
        state: "hidden",
