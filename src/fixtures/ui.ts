...
function getLiveTestTimerIntervalMs(): number {
  ...
  return Number.isFinite(parsedInterval) && parsedInterval >= 1_000
    ? parsedInterval
    : DEFAULT_LIVE_TEST_TIMER_INTERVAL_MS;
}
...
function isKnownBenignApiError(
  url: string,
  method: string,
  status: number,
): boolean {
...
function buildBoundedApiDetails(entries: string[], maxChars: number): string {
  ...
  return overflow > 0
    ? `${details} | +${overflow} more (see failure-data.json)`
    : details;
}
...
function buildTimeoutSuspects(
  ...
  const timeoutFailures = failedRequests.filter((request) =>
    /timeout|timed out|ETIMEDOUT/i.test(request.errorText),
  );
  const suspectsFromFailed = timeoutFailures.map(
    (request) =>
      `${request.method} ${request.url} -> REQUEST_FAILED (${stripAnsi(request.errorText).substring(0, 120)})`,
  );
  const suspectsFromSlow = slowCalls.map(
    (call) =>
      `${call.method} ${call.url} -> SLOW (${Math.round(call.duration)}ms)`,
  );
...
function buildTopSuspect(
  ...
  const summarizedFailedCalls = summarizeApiFailures(
    serverErrors,
    clientErrors,
    failedRequests,
  );
...
function hasStrongSlowBackendSignal(
  slowCalls: Array<{ url: string; duration: number; method: string }>,
): boolean {
  if (slowCalls.length >= STRONG_SLOW_CALL_COUNT) {
    return true;
  }
  return slowCalls.some(
    (call) => call.duration >= STRONG_SLOW_CALL_DURATION_MS,
  );
}
...
function deriveBackendWaitFlag(
  ...
  const hasTimeoutFailedRequest = failedRequests.some((request) =>
    /timeout|timed out|ETIMEDOUT/i.test(request.errorText),
  );
...
function isSlowOrNetworkTimeout(failureType: FailureType): boolean {
  return (
    failureType === "SLOW_API_RESPONSE" || failureType === "NETWORK_TIMEOUT"
  );
}
...
function deriveTimeoutPhaseMarker(
  ...
  return executionSignals.backendRequestsObserved > 0
    ? "ui-timeout-post-backend"
    : "ui-timeout-pre-backend";
}
...
function collectErrorContext(testInfo: TestInfo): ErrorContext {
  ...
  const diagnosticText = (
    allErrorEntries.length > 0 ? allErrorEntries : [fallbackEntry]
  )
    .filter(Boolean)
    .join("\n\n---\n\n");
  ...
  return {
    ...
    actionableLine: extractActionableErrorLine(
      diagnosticText || primaryMessage,
    ),
  };
}
...
function deriveTimeoutRootCause({
  ...
  const locationHint =
    failureLocation && failureLocation !== "unknown:0:0"
      ? ` Location: ${failureLocation}.`
      : "";
  const actionableHint = actionableErrorLine
    ? ` Last actionable error: ${actionableErrorLine}.`
    : "";
...
function createResponseHandler(apiErrors: ApiError[], maxTracked: number) {
  return async (response: Response) => {
    try {
      ...
      if (
        status >= 400 &&
        !isKnownBenignApiError(sanitizedUrl, method, status)
      ) {
        apiErrors.push({
          url: sanitizedUrl,
          status,
 ...
 */
function createRequestFailedHandler(
  failedRequests: FailedRequest[],
  maxTracked: number,
  networkTimeoutRef: { value: boolean },
) {
  return (request: Request) => {
    const url = request.url();
    if (isBackendApi(url)) {
      ...
      if (
        failure?.errorText?.includes("Timeout") ||
        failure?.errorText?.includes("timeout")
      ) {
...
async function attachFailureDiagnosis(
  ...
  const clientErrors = apiErrors.filter(
  ...
  const slowEndpointSummary = slowCallAggregates
    .slice(0, 3)
  ...
  const phaseMarker = derivePhaseMarker(
    failureType,
    error,
    executionSignals,
    backendWait,
  ...
  const slowEndpointDetails = buildBoundedApiDetails(
    slowEndpointSummary,
    MAX_API_ERRORS_DETAILS_CHARS,
  );
  ...
  if (
    ...
    failureType === "DOWNSTREAM_API_4XX" ||
    ...
    const boundedApiDetails = buildBoundedApiDetails(
      summarizedFailedCalls,
      MAX_API_ERRORS_DETAILS_CHARS,
    );
    testInfo.annotations.push({
      type: "API errors",
    ...
    if (boundedApiDetails) {
      testInfo.annotations.push({
        type: "API errors (details)",
        description: boundedApiDetails,
      });
    }
  }
