...
test.describe(
  ...
  () => {
    ...
    test("can continue when choosing an existing booking", async ({ bookingUiPage, page }) => {
    ...
    test("can continue when creating a new booking", async ({ bookingUiPage, page }) => {
      await test.step("Navigate to booking UI and wait for bookings request", async () => {
        await bookingUiPage.goto();
        await expect(page).toHaveURL(bookingPageUrlPattern);
        await expect.poll(() => getBookingsCalled).toBeTruthy();
      ...
      await test.step("Intercept create booking API request and verify payload", async () => {
        ...
        expect(bookingResponse.endTime).toBe(
          new Date(new Date(submittedRequest.endDate).getTime() + 1).toISOString()
        );
    ...
    test("can continue when viewing tasks and cases", async ({ bookingUiPage, page }) => {
      await test.step("Navigate to booking UI and wait for bookings request", async () => {
        await bookingUiPage.goto();
        await expect(page).toHaveURL(bookingPageUrlPattern);
        await expect.poll(() => getBookingsCalled).toBeTruthy();
