...
test.describe(
  ...
  { tag: ["@integration-bucket-2", "@integration", "@integration-event-behaviour"] },
  () => {
    ...
    test("stays on check-your-answers when event creation returns a backend error", async ({ caseDetailsPage, page }) => {
      await openEventBehaviourJourney(page, caseDetailsPage, {
        submit: { status: 500, body: { message: "event-submit-failed" } }
      });
      await caseDetailsPage.selectCaseAction(EVENT_BEHAVIOUR_TRIGGER_NAME, {
        expectedLocator: page.getByLabel("Outcome type")
      });
      ...
      const failedResponse = page.waitForResponse(
        (response) => response.url().includes("/events") && response.request().method() === "POST" && response.status() === 500
      );
      ...
      await expect(page.getByText("event-submit-failed")).toBeVisible();
    });

    test("stays on check-your-answers when event creation returns a client error", async ({ caseDetailsPage, page }) => {
      await openEventBehaviourJourney(page, caseDetailsPage, {
        submit: { status: 400, body: { message: "event-submit-invalid" } }
      });
      await caseDetailsPage.selectCaseAction(EVENT_BEHAVIOUR_TRIGGER_NAME, {
        expectedLocator: page.getByLabel("Outcome type")
      });
      ...
      const failedResponse = page.waitForResponse(
        (response) => response.url().includes("/events") && response.request().method() === "POST" && response.status() === 400
      );
      await caseDetailsPage.submitCaseEvent();
      await failedResponse;
      ...
      await expect(caseDetailsPage.checkYourAnswersHeading).toBeVisible();
      await expect(caseDetailsPage.eventCreationErrorHeading).toBeVisible();
      await expect(page.getByText("event-submit-invalid")).toBeVisible();
    });

    test("stays on check-your-answers when event creation times out", async ({ caseDetailsPage, page }) => {
      await openEventBehaviourJourney(page, caseDetailsPage, {
        submit: { abortErrorCode: "timedout" }
      });
      await caseDetailsPage.selectCaseAction(EVENT_BEHAVIOUR_TRIGGER_NAME, {
        expectedLocator: page.getByLabel("Outcome type")
      });
      ...
      const submitRequest = page.waitForRequest(
        (request) => request.url().includes("/events") && request.method() === "POST"
      );
      await caseDetailsPage.submitCaseEvent();
      await submitRequest;
