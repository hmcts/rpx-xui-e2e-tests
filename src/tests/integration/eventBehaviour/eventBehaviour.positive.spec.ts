  ...
  () => {
    ...
    test("renders a configured multi-page event and submits the exact event payload", async ({
      ...
      const submitRequest = page.waitForRequest(
        (request) =>
          request.url().includes(`/data/cases/${EVENT_BEHAVIOUR_CASE_REFERENCE}/events`) &&
          request.method() === "POST"
      );
      const submitResponse = page.waitForResponse(
        (response) =>
          response.url().includes(`/data/cases/${EVENT_BEHAVIOUR_CASE_REFERENCE}/events`) &&
