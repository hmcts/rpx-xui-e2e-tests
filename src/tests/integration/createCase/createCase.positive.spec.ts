...
test.beforeEach(async ({ page, createCasePage }) => {
  ...
  await page.route(
    `**/data/internal/case-types/${caseType}/event-triggers/createCase*`,
    async (route) => {
  ...
  () => {
    test(`All expected fields are filled, seen, and sent in the create case API call`, async ({
      ...
      await test.step("Check the answers shown match the entered data", async () => {
        const answerTable = await caseDetailsPage.trRowsToObjectInPage(
          createCasePage.checkYourAnswers,
        );
        ...
        const person1 = await caseDetailsPage.trRowsToObjectInPage(
          await createCasePage.findTableInCheckAnswers("Person 1"),
        );
        const person2 = await caseDetailsPage.trRowsToObjectInPage(
          await createCasePage.findTableInCheckAnswers("Person 2"),
        );
        ...
        expect(jobSubTable2).toMatchObject({
      ...
      await test.step("Submit the case for creation and capture the request body", async () => {
        const interceptedCreateCaseRequestBodyPromise =
          routeCaseCreationFlow(page);
        await createCasePage.testSubmitButton.click({ noWaitAfter: true });
        interceptedCreateCaseRequestBody =
          await interceptedCreateCaseRequestBodyPromise;
      });
      ...
      await test.step("Check the JSON sent in the creation request matches the expected data", async () => {
        ...
        expect(submittedData.DivorceReason[0]).toEqual(
          caseData.divorceReasons?.[0]?.toLowerCase(),
        );
        ...
        expect(submittedData.Person1?.PersonGender).toBe(
          person1Data.gender?.toLowerCase(),
        );
        expect(submittedData.Person1?.PersonJob?.Title).toBe(
          person1Data.jobTitle,
        );
        expect(submittedData.Person1?.PersonJob?.Description).toBe(
          person1Data.jobDescription,
        );
        ...
        expect(submittedData.Person2?.PersonGender).toBe(
          person2Data.gender?.toLowerCase(),
        );
        expect(submittedData.Person2?.PersonJob?.Title).toBe(
          person2Data.jobTitle,
        );
        expect(submittedData.Person2?.PersonJob?.Description).toBe(
          person2Data.jobDescription,
        );
      });
    });
    ...
    test(`Creating a case with hidden and omitted fields, shows the expected answers and JSON request body`, async ({
      ...
      await test.step("Check the answers shown match the expected visible fields", async () => {
        const table = await caseDetailsPage.trRowsToObjectInPage(
          createCasePage.checkYourAnswersTable,
        );
    ...
    test(`When you change the gender of Person 1, the previously filled 'maiden name' field is hidden, and not sent in the API request`, async ({
      ...
      await test.step("Check the form matches previously entered data", async () => {
        ...
        await expect(createCasePage.person1FirstNameInput).toHaveValue(
          person1Data.firstName ?? "",
        );
        await expect(createCasePage.person1MaidenNameInput).toHaveValue(
          person1Data.maidenName ?? "",
        );
        await expect(createCasePage.person1LastNameInput).toHaveValue(
          person1Data.lastName ?? "",
        );
        await expect(createCasePage.person1TitleInput).toHaveValue(
          person1Data.title ?? "",
        );
        expect(await createCasePage.person1GenderSelect.inputValue()).toContain(
          person1Data.gender?.toLowerCase(),
        );
        await expect(createCasePage.person1JobTitleInput).toHaveValue(
          person1Data.jobTitle ?? "",
        );
        await expect(createCasePage.person1JobDescriptionInput).toHaveValue(
          person1Data.jobDescription ?? "",
        );
      });
      ...
      await test.step("Update the gender to male, wait for maiden name to be hidden, advance to the check your answers page", async () => {
        ...
        await createCasePage.person1MaidenNameInput.waitFor({
          state: "hidden",
        });
        // Defensive clear: some environments retain the hidden field value in form state.
        await page.evaluate(() => {
          const maidenNameInput = document.querySelector(
            "#Person1_MaidenName",
          ...
          maidenNameInput.dispatchEvent(new Event("input", { bubbles: true }));
          maidenNameInput.dispatchEvent(new Event("change", { bubbles: true }));
        });
        await createCasePage.clickContinueAndWait(
          "after changing gender to male",
        );
        await createCasePage.clickContinueAndWait(
          "advancing to check your answers after changing person 1 gender",
        );
      });
      ...
      await test.step(`Check the JSON sent in the creation request doesn't contain 'maiden name' and shows the updated gender field`, async () => {
        ...
        expect(submittedData.Person1?.MaidenName ?? "").toBe("");
      });
    });
  },
);
