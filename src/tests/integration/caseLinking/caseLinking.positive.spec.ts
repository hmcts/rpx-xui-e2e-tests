...
import { openCaseLinkingJourney } from "../helpers/index.js";
import {
...
test.describe(
  ...
  () => {
    test("links a case from case details and submits the selected reason", async ({
      ...
      await test.step("Submit the link-cases event and assert the CCD request payload", async () => {
        const submitRequest = page.waitForRequest(
          (request) =>
            request.url().includes(`/data/cases/${CASE_LINKING_CASE_REFERENCE}/events`) &&
            request.method() === "POST"
        );
      ...
      await test.step("Return to case details and show the linked case in the Linked cases tab", async () => {
        await expect(page).toHaveURL(
          new RegExp(`/cases/case-details/.*/.*/${CASE_LINKING_CASE_REFERENCE}(?:$|#)`)
        );
        ...
        const linkedCaseLink = linkedCasesPanel
          .getByRole("link", {
            name: formatCaseReferenceForDisplay(CASE_LINKING_RELATED_CASE_REFERENCE)
          })
          .first();
        await expect(linkedCaseLink).toBeVisible();
        await expect(linkedCaseLink).toHaveAttribute(
          "href",
          new RegExp(`/v2/case/${CASE_LINKING_RELATED_CASE_REFERENCE}$`)
        );
      });
    });

    test("allows a judiciary user to open the Link cases event from case details", async ({
      caseDetailsPage,
      page
    }) => {
      await openCaseLinkingJourney(page, caseDetailsPage, {
    ...
    test("shows linked cases in their supplied order on the Linked cases tab", async ({
      caseDetailsPage,
      page
    }) => {
      await openCaseLinkingJourney(page, caseDetailsPage, {
      ...
      await expect(firstLinkedCaseLink).toHaveAttribute(
        "href",
        new RegExp(`/v2/case/${CASE_LINKING_RELATED_CASE_REFERENCE}$`)
      );
    ...
    test("submits the Other case-link reason with a custom description", async ({
      caseDetailsPage,
      page
    }) => {
      await openCaseLinkingJourney(page, caseDetailsPage);
