...
test.describe(
  ...
  () => {
    ...
    test("shows a backend error when the user tries to link the case to itself", async ({
      caseDetailsPage,
      page
    }) => {
      await openCaseLinkingJourney(page, caseDetailsPage, {
      ...
      await test.step(
        `Open Link cases and continue with self-link reference ${CASE_LINKING_CASE_REFERENCE} using reason ${CASE_LINKING_REASON_LABEL}`,
        async () => {
          await caseDetailsPage.openLinkCasesEvent();
      ...
      await test.step(
        'Submit and verify the 400 self-link error "A case cannot be linked to itself"',
        async () => {
    ...
    test("shows an error and stays on check your answers when the case-link submit fails", async ({
      ...
      await test.step(
        `Open Link cases and continue with linked case ${CASE_LINKING_RELATED_CASE_REFERENCE} using reason ${CASE_LINKING_REASON_LABEL}`,
        async () => {
          await caseDetailsPage.openLinkCasesEvent();
          await caseDetailsPage.fillCaseLinkDetails({
      ...
      await test.step(
        'Submit and verify the 500 error "case-link-submit-failed" while remaining on check your answers',
        async () => {
          await caseDetailsPage.submitCaseEvent();
          await failedResponse;
