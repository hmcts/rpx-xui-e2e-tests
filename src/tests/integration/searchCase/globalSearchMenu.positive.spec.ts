...
test.beforeEach(async ({ page }) => {
  await setupGlobalSearchMockRoutes(page, {
    ...
    caseDetailsHandler: async (route) => {
      ...
      const caseReference =
        requestUrl.split("/").pop() ?? GLOBAL_SEARCH_CASE_REFERENCE;
      await route.fulfill({
        status: 200,
...
test.describe(`Global search from menu bar as ${userIdentifier}`, () => {
  test("searches by 16-digit case reference and navigates to case details", async ({
    ...
    await test.step("Verify search result exposes case details link and navigates", async () => {
  ...
  test("shows no results content for non-existent 16-digit case reference", async ({
    ...
    await test.step("Verify no-results content and key action link", async () => {
      ...
      await expect(
        page.getByRole("button", { name: "Search again", exact: true }),
      ).toBeVisible();
    });
  });
});
