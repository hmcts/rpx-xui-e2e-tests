...
export function getCaseReferenceFromFindCaseSearchPayload(
  payload: unknown,
): string | undefined {
  ...
  return payloadJson
    ? extractSixteenDigitCaseReference(payloadJson)
    : undefined;
}
...
function findCaseReferenceInObject(
...
function findDirectCaseReferenceInObject(
  payloadRecord: Record<string, unknown>,
): string | undefined {
  ...
  if (
    ...
    isCaseReferenceIdentifier(payloadRecord.id) &&
    "value" in payloadRecord
  ) {
    const normalizedCaseReference = normalizeCaseReferenceValue(
      payloadRecord.value,
    );
    if (normalizedCaseReference) {
      return normalizedCaseReference;
...
function findNestedCaseReferenceInObject(
  payloadRecord: Record<string, unknown>,
): string | undefined {
...
function isCaseReferenceIdentifier(identifier: string): boolean {
  ...
  return (
    normalizedIdentifier === "casereference" ||
    normalizedIdentifier === "ccdcasereference"
  );
}
