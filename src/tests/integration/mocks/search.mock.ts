import { TEST_CASE_REFERENCES } from "../testData/index";

/** Single source of truth: sourced from testData to avoid cross-file duplication */
export const VALID_SEARCH_CASE_REFERENCE =
  TEST_CASE_REFERENCES.QUICK_SEARCH_VALID;
export const INVALID_SEARCH_CASE_REFERENCE =
  TEST_CASE_REFERENCES.QUICK_SEARCH_INVALID;
...
export function buildGlobalSearchResultsMock(
  caseReference: string = VALID_SEARCH_CASE_REFERENCE,
) {
  return {
    resultInfo: {
      caseStartRecord: 1,
...
export function buildCaseDetailsMock(
  caseReference: string = VALID_SEARCH_CASE_REFERENCE,
) {
  return {
    _links: {
