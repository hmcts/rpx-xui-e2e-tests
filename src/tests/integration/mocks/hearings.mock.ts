...
type HearingsUserDetailsMock = {
  ...
  roleAssignmentInfo: Array<Record<string, string>>;
};
...
export function buildHearingsUserDetailsMock(roles: string[]): HearingsUserDetailsMock {
  return {
    sessionTimeout: {
...
export function buildLovRefDataMock(
  ...
  const baselineHearingTypeItems = (hearingStageRefData ?? [])
...
export function buildHearingActualsMock() {
  return deepClone(hearingActualsMainModel) as UnknownRecord;
}

function resolveHearingLinks() {
  return deepClone(hearingLinksState) as UnknownRecord;
}
