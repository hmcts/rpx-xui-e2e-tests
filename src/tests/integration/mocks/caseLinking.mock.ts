...
const caseWithoutLinksTemplate = require("./assets/caseLinking/getCaseNoLinkedCases.json");
const linkedCasesResponseTemplate = require("./assets/caseLinking/getLinkedCases.json");
...
function normalizeCaseReferenceMetadata(caseDetails: Record<string, unknown>, caseReference: string): void {
  ...
  const caseReferenceMetadataField = metadataFields.find(
    (field): field is Record<string, unknown> =>
      typeof field === "object" && field !== null && field.id === "[CASE_REFERENCE]"
  );
...
export const CASE_LINKING_LINKED_CASES_API_CASE_TYPE = String(
  linkedCasesResponseTemplateFirstItem?.ccdCaseType ?? "benefit"
);
...
export function buildCaseLinkingLinkedCasesResponseMock(options?: { linkedCases?: CaseLinkingLinkedCase[] }) {
  ...
  linkedCasesResponse.hasMoreRecords = "False";
