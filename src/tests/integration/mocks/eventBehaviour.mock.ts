...
const caseDetailsTemplate = require("./assets/caseLinking/getCaseNoLinkedCases.json") as Record<string, unknown>;
...
export function buildEventBehaviourCaseDetails(
  options: { eventRecorded?: boolean; eventAvailable?: boolean } = {}
) {
  ...
  caseDetails.triggers = options.eventAvailable === false ? [] : [
    {
      id: EVENT_BEHAVIOUR_TRIGGER_ID,
      name: EVENT_BEHAVIOUR_TRIGGER_NAME,
      description: "Record a configured case outcome",
