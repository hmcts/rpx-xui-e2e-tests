...
export function buildCaseListMock(rowCount = 2) {
...
export function buildCaseListMockForPage(totalResults: number, pageNumber: number, pageSize = 25) {
...
export function buildCaseListMockForDefaultState(state: string) {
  ...
  const normalizedState = state === "Any" ? "CaseCreated" : state;

  mock.results.forEach((result) => {
    result.case_fields["[STATE]"] = normalizedState;
...
function applyOptionalFieldOverrides<T extends { results: Array<{ case_fields: Record<string, string> }> }>(
  mock: T
): T {
  if (mock.results[0]) {
...
export function buildCaseListStateFilterInputsMock() {
  ...
  const localAuthorityInput = buildWorkbasketInput(
    "Local authority",
    "LocalAuthority",
    buildFixedListFieldType("MultiSelectList-local-authority", "MultiSelectList", [
      "Authority 1",
      "Authority 2"
    ])
  );
...
export function buildCaseListMockWithOptionalFields(rowCount = 2) {
  return applyOptionalFieldOverrides(buildCaseListMock(rowCount));
}

export function buildCaseListMockWithOptionalFieldsForPage(
  totalResults: number,
  pageNumber: number,
  pageSize = 25
) {
