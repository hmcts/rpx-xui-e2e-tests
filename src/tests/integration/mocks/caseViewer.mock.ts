...
const caseTemplate = require("./assets/caseLinking/getCaseNoLinkedCases.json");
...
function viewerField(
  id: string,
  label: string,
  type: "CaseHistoryViewer" | "CasePaymentHistoryViewer",
  value: unknown
) {
  return {
...
export function buildCaseViewerMock(variant: CaseViewerVariant = "populated") {
  ...
  mock.metadataFields = mock.metadataFields.map((field) =>
