...
export async function routeMyTaskActionFlow(
  ...
  await page.route(
    `**/workallocation/task/${taskId}/roles*`,
    async (route: Route) => {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
  ...
  await page.route(
    `**/workallocation/task/${taskId}*`,
    async (route: Route) => {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify({ task: taskWithActions }),
      });
    },
  );
  ...
  await page.route(
    `**/workallocation/task/${taskId}/${actionId}*`,
    async (route: Route) => {
      actionInvoked = true;
...
export async function routeMyTaskCancellationFlow(
  ...
  const { getActionRequestUrl, getActionRequestBody } =
    await routeMyTaskActionFlow(page, taskId, task, {
      actionId: "cancel",
...
export async function routeCaseDetailsTaskActionFlow(
  ...
  await page.route(
    `**/workallocation/case/task/${scenario.caseId}*`,
    async (route: Route) => {
  ...
  await page.route(
    `**/api/workallocation/case/task/${scenario.caseId}*`,
    async (route: Route) => {
      const tasks = actionInvoked ? [] : [task];
      await route.fulfill({
        status: 200,
...
export async function routeCaseDetailsTaskCancellationFlow(
  ...
  await routeCaseDetailsTaskActionFlow(
    page,
    taskId,
    scenario,
    task,
