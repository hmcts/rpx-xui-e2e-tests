...
function highlightExpectedPairsInActual(
  ...
  for (const [key, value] of Object.entries(expectedPayload)) {
    ...
    const pairRegex = new RegExp(
      `"${escapeRegExp(key)}"\\s*:\\s*${escapeRegExp(valueAsJson)}`,
      "g",
    );
...
export function logTaskCancellationAssertion(
  input: CancellationAssertionInput,
): CancellationAssertionResult {
  ...
  const payloadMatch =
    JSON.stringify(input.expectedPayload) ===
    JSON.stringify(input.actualPayload);
  ...
  const actualPayloadWithHighlights = highlightExpectedPairsInActual(
