...
import { CASE_VIEWER_CASE_REFERENCE } from "../mocks/caseViewer.mock.js";
...
test.describe("CCD case viewer regression coverage", { tag: ["@integration", "@integration-case-viewer"] }, () => {
  test("renders configured viewer tabs and case data", async ({ page, caseDetailsPage }) => {
    ...
    await expect(await caseDetailsPage.getCaseViewerRowByName("Case reference")).toBeVisible();
    await expect(page).toHaveURL(new RegExp(CASE_VIEWER_CASE_REFERENCE));
  });
