...
test.describe(
  `Available Task List as ${userIdentifier}`,
  ...
  () => {
    test(`User ${userIdentifier} can view a large number of available tasks on the task list page`, async ({
      ...
      await test.step("Verify the UI shows the expected layout and data, given the mock response", async () => {
        ...
        for (let i = 0; i < table.length; i++) {
          ...
          expect
            .soft(table[i]["Case category"])
            .toBe(taskListMockResponse.tasks[i].case_category);
          expect
            .soft(table[i]["Location"])
            .toBe(taskListMockResponse.tasks[i].location_name);
          expect
            .soft(table[i]["Task"])
            .toBe(taskListMockResponse.tasks[i].task_title);
          expect
            .soft(table[i]["Due date"])
            .toBe(formatUiDate(taskListMockResponse.tasks[i].due_date));
          // Hearing date: allow empty string or null
          const expectedHearingDate =
            taskListMockResponse.tasks[i].next_hearing_date || "";
          expect
            .soft(table[i]["Hearing date"])
            .toBe(formatUiDate(expectedHearingDate));
        }
      });

      await test.step("Verify the pagination controls behave and display as expected for large result sets", async () => {
        await expect
          .soft(taskListPage.paginationPreviousButton)
          .not.toBeVisible();
        ...
        expect
          .soft(await taskListPage.paginationCurrentPage.textContent())
          .toContain("1");
        ...
        expect
          .soft(await taskListPage.getResultsText())
          .toBe(
            `Showing 26 to 50 of ${taskListMockResponse.total_records} results`,
          );
      });
    });

    test(`User can view a small number of assigned tasks on the task list page`, async ({
      taskListPage,
      page,
      tableUtils,
    }) => {
    ...
    test(`User sees the no tasks message if the api returns an empty response`, async ({
      taskListPage,
      page,
    }) => {
    ...
    test(`User can see all table sorting buttons on the table`, async ({
      taskListPage,
      page,
    }) => {
      const emptyMockResponse = { tasks: [], total_records: 0 };
    ...
    test(`User ${userIdentifier} can assign a task to themselves and see the expected notifications`, async ({
      ...
      await test.step("Verify you can assign test", async () => {
        ...
        await expect(
          taskListPage.exuiBodyComponent.successMessage,
        ).toContainText(
          `You've assigned yourself a task. It's available in My tasks.`,
        );
        await expect(taskListPage.exuiBodyComponent.infoMessage).toContainText(
