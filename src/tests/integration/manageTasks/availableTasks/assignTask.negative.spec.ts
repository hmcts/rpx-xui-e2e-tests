...
test.describe(
  ...
  () => {
    test(`User sees service down when claim task endpoint returns 500`, async ({
      taskListPage,
      page,
    }) => {
      const firstTask = taskListMockResponse.tasks[0];
      let sawClaimPost = false;
      ...
      await test.step("Setup route mocks for task list and claim 500 response", async () => {
        ...
        await page.route(
          `**/workallocation/task/${firstTask.id}/claim*`,
          async (route) => {
      ...
      await test.step("Attempt claim action and verify service down", async () => {
        await taskListPage.openFirstManageActions(
          "available tasks claim 500 response",
        );
        ...
        await expect.poll(() => sawClaimPost).toBe(true);
    ...
    test(`User sees task unavailable warning when claim endpoint returns 400`, async ({
      taskListPage,
      page,
    }) => {
      const firstTask = taskListMockResponse.tasks[0];
      ...
      await test.step("Attempt claim action and verify task unavailable warning", async () => {
        await taskListPage.openFirstManageActions(
          "available tasks claim 400 response",
        );
        await expect(taskListPage.taskActionClaim).toBeVisible();
        await taskListPage.clickTaskAction(
          taskListPage.taskActionClaim,
        ...
        await expect(taskListPage.taskListTable).toBeVisible();
        await expect(
          taskListPage.exuiBodyComponent.warningMessage,
        ).toContainText(TASK_UNAVAILABLE_WARNING);
