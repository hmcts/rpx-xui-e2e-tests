...
const broaderSupportedJurisdictionsMock = [
  "IA",
  "PRIVATELAW",
  "PUBLICLAW",
  "CIVIL",
...
test.describe(
  ...
  () => {
    test(`User ${userIdentifier} sees filter errors if no services are selected`, async ({
      taskListPage,
      page,
    }) => {
      ...
      await test.step("Setup route mock for task list", async () => {
        await setupTaskListBootstrapRoutes(
          page,
          broaderSupportedJurisdictionsMock,
        );
        await page.route(
      ...
      await test.step("Verify table shows filter errors if no services are selected", async () => {
        ...
        await expect(taskListPage.selectServicesError).toContainText(
          "Select a service",
        );
      });
    });

    test(`User ${userIdentifier} sees filter errors if no types of work are selected`, async ({
      taskListPage,
      page,
    }) => {
      const taskListMockResponse = buildTaskListMock(
      ...
      await test.step("Verify table shows filter errors if no types of work are selected", async () => {
        ...
        await expect(taskListPage.selectTypesOfWorksError).toContainText(
    ...
    test(`User sees the no tasks message on available tasks, if the api times out`, async ({
      taskListPage,
      page,
    }) => {
      await test.step("Setup route mock for empty task list", async () => {
