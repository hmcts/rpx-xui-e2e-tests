...
test.describe(`User ${userIdentifier} can see assigned tasks on a case`, () => {
  test(`Low priority tasks assigned to logged in user show elements and markdown as expected`, async ({
    ...
    await test.step("Verify the task table shows the expected results", async () => {
      ...
      expect
        .soft(
          table[0]["Next steps HTML"],
          "Next steps link does not contain expected submitAppeal href",
        )
        .toContain(
          `href="/case/IA/Asylum/${caseId}/trigger/submitAppeal?tid=${taskData.id}"`,
        );
      expect
        .soft(table[1]["Next steps"], "Next steps not showing expected text")
        .toContain("Current progress of the case");
      expect
        .soft(
          table[1]["Next steps HTML"],
          "Next steps not showing expected image",
        )
        .toContain("progress_legalRep_appealStarted.svg");
      expect
        .soft(
          table[1]["Next steps HTML"],
          "Next steps image alt text is missing",
        )
        .toContain(
          "Progress map showing that the appeal is now at stage 1 of 11 stages - the Appeal started stage",
        );
      expect
        .soft(
          table[2]["Next steps HTML"],
          "Next steps not showing text as a heading",
        )
        .toContain("<h1>Next steps</h1>");
      expect
    ...
    await test.step("Verify the expected priority labels are shown", async () => {
      ...
      taskData.titles.forEach((title, i) => {
        ...
        expect
          .soft(
            table[i]["Priority"],
            `The priority label for task ${i} should be ${expectedPriority}`,
          )
          .toContain(expectedPriority);
        expect
          .soft(table[i]["Due date"], `The due date for task ${i} should match`)
          .toBe(formatUiDate(taskData.dueDates[i]));
        expect
          .soft(table[i]["Title"], `The title for task ${i} should match`)
          .toContain(title);
        expect
          .soft(
            table[i]["Next steps"],
            `Next steps for task ${i} should contain CTA`,
          )
  ...
  test(`Tasks not assigned to the logged in user don't show next steps`, async ({
    caseDetailsPage,
    page,
  }) => {
  ...
  test(`Tasks with task alerts show above all tasks`, async ({
    caseDetailsPage,
    page,
  }) => {
    const otherUser = faker.string.uuid();
    ...
    await test.step("Verify task alerts are shown in the UI", async () => {
      ...
      expect
        .soft(alertText)
        .toContain(taskData.warning_list[0].values[0].warningText);
      expect
        .soft(alertText)
        .toContain(taskData.warning_list[0].values[1].warningText);
      expect
        .soft(alertText)
        .toContain(taskData.warning_list[1].values[0].warningText);
      expect
        .soft(alertText)
        .toContain(taskData.warning_list[1].values[1].warningText);
    });
  });

  test(`Complex Markdown label renders correctly`, async ({
    caseDetailsPage,
    page,
  }) => {
    const taskData = {
    ...
    await test.step("Verify Next steps elements are shown as expected", async () => {
      ...
      expect.soft(content[0]["Next steps"]).toContain("Overview");
      expect
        .soft(nextStepsHtml)
        .toContain("progress_legalRep_appealStarted.svg");
      expect
      ...
      expect
        .soft(nextStepsHtml)
        .toContain(`/case/IA/Asylum/${caseId}/trigger/submitAppeal`);
      expect
        .soft(nextStepsHtml)
        .toContain(`/case/IA/Asylum/${caseId}/trigger/editAppeal`);
    });
  });
});
