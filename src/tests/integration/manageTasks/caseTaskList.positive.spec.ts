...
test.describe(`User ${userIdentifier} can see assigned tasks on a case`, () => {
  test(`Low priority tasks assigned to logged in user show elements and markdown as expected`, async ({
    ...
    await test.step('Verify the task table shows the expected results', async () => {
      ...
      expect.soft(table[1]['Next steps HTML'], 'Next steps not showing expected image').toContain(
        'progress_legalRep_appealStarted.svg'
      );
      expect.soft(table[1]['Next steps HTML'], 'Next steps image alt text is missing').toContain(
  ...
  test(`Complex Markdown label renders correctly`, async ({ caseDetailsPage, page }) => {
    ...
    await test.step('Verify Next steps elements are shown as expected', async () => {
      ...
      expect.soft(nextStepsHtml).toContain('progress_legalRep_appealStarted.svg');
      expect
        .soft(nextStepsHtml)
        .toContain('Progress map showing that the appeal is now at stage 1 of 11 stages - the Appeal started stage');
      expect
      ...
      expect.soft(nextStepsHtml).toContain(`/case/IA/Asylum/${caseId}/trigger/submitAppeal`);
      expect.soft(nextStepsHtml).toContain(`/case/IA/Asylum/${caseId}/trigger/editAppeal`);
    });
  });
});
