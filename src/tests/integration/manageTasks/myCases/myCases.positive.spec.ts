...
test.describe(`My Cases as ${userIdentifier}`, { tag: ['@integration-bucket-3', '@integration', '@integration-manage-tasks'] }, () => {
  test(`Cases and actions menus render correctly`, async ({ taskListPage, page, tableUtils }) => {
    ...
    await test.step('Verify the cases table columns and data as seen as expected', async () => {
      await expect(taskListPage.myCasesResultsAmount).toHaveText(`Showing ${myCasesMockResponse.cases.length} results`);
