...
import {
  ...
  singleUsersGetByRoleMockResponse,
...
test.describe(
  `Task Reassign as ${userIdentifier}`,
  { tag: ["@integration", "@integration-manage-tasks"] },
  () => {
    test(`User can reassign a task from the task list`, async ({
      taskListPage,
      page,
    }) => {
      ...
      await test.step("Navigate to task list and open reassign action", async () => {
        ...
        await taskListPage.clickTaskAction(
          taskListPage.taskActionReassign,
          "my tasks reassign action",
        );

        await expect(page).toHaveURL(
          new RegExp(`/work/${firstTask.id}/reassign`),
        );
      ...
      await test.step("Submit reassign action and verify expected API response", async () => {
        const expectedReassignUserId =
          singleUsersGetByRoleMockResponse[0].idamId;
        ...
        const requestBody = reassignRequest.postDataJSON() as {
          userId?: string;
        };
