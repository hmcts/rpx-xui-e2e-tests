...
test.describe(
  `Task Completion as ${userIdentifier}`,
  { tag: ["@integration", "@integration-manage-tasks"] },
  () => {
    test(`User can mark one of their assigned tasks as done`, async ({
      taskListPage,
      page,
    }) => {
      ...
      await test.step("Navigate to task list and open complete action", async () => {
        ...
        await taskListPage.waitForManageButton("my tasks complete action");
      ...
      await test.step("Submit complete action and verify request/response", async () => {
        ...
        expect(completeRequest.postDataJSON()).toEqual({
          hasNoAssigneeOnComplete: false,
        });
