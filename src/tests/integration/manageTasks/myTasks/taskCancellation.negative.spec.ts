...
test.describe(
  ...
  () => {
    test(`User sees service down when cancel endpoint returns 500`, async ({
      taskListPage,
      page,
    }) => {
      ...
      await test.step("Setup route mocks for list, action dependencies, and cancel 500 response", async () => {
        ...
        await page.route(
          `**/workallocation/task/${firstTask.id}/cancel*`,
          async (route) => {
      ...
      await test.step("Attempt cancel action and verify service down", async () => {
        ...
        await taskListPage.waitForManageButton("my tasks cancel 500 response");

        await taskListPage.openFirstManageActions(
          "my tasks cancel 500 response",
        );
        await expect(taskListPage.taskActionCancel).toBeVisible();
        await taskListPage.clickTaskAction(
          taskListPage.taskActionCancel,
          "my tasks cancel 500 response",
        );
        await expect(page).toHaveURL(
          new RegExp(`/work/${firstTask.id}/cancel`),
        );
        ...
        const cancelFailureResponsePromise = page.waitForResponse(
          (response) =>
            ...
            response
              .url()
              .includes(`/workallocation/task/${firstTask.id}/cancel`) &&
            response.status() === 500,
        );
    ...
    test(`User returns to my tasks with warning when cancel endpoint returns 400`, async ({
      taskListPage,
      page,
    }) => {
      const firstTask = taskListMockResponse.tasks[0];
      ...
      await test.step("Attempt cancel action and verify redirect to my tasks with warning alert", async () => {
        await taskListPage.goto();
        await taskListPage.exuiSpinnerComponent.wait();
        await taskListPage.waitForManageButton("my tasks cancel 400 response");

        await taskListPage.openFirstManageActions(
          "my tasks cancel 400 response",
        );
        await expect(taskListPage.taskActionCancel).toBeVisible();
        await taskListPage.clickTaskAction(
          taskListPage.taskActionCancel,
