...
test.describe(`Task Go To negative scenarios as ${userIdentifier}`, { tag: ['@integration-bucket-3', '@integration', '@integration-manage-tasks'] }, () => {
  [500, 400].forEach((statusCode) => {
    test(`User still reaches case details when go endpoint returns ${statusCode}`, async ({ taskListPage, page }) => {
      ...
      await expect(page.getByRole('heading', { name: /there is a problem/i })).toHaveCount(0);
    });
  });
});
