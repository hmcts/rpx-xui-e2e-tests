...
test.describe(
  ...
  () => {
    [500, 400].forEach((statusCode) => {
      test(`User still reaches case details when go endpoint returns ${statusCode}`, async ({
        taskListPage,
        page,
      }) => {
        ...
        await test.step(`Setup route mocks for list, go dependencies, and case task ${statusCode} response`, async () => {
          ...
          await page.route(
            `**/workallocation/case/task/${firstTask.case_id}*`,
            async (route) => {
          ...
          await page.route(
            `**/data/internal/cases/${firstTask.case_id}*`,
            async (route) => {
        ...
        await test.step(`Attempt go action and verify behavior for ${statusCode}`, async () => {
          ...
          await taskListPage.waitForManageButton(
            `my tasks go to ${statusCode} response`,
          );
          ...
          const goFailureResponsePromise = page.waitForResponse(
            (response) =>
              ...
              response
                .url()
                .includes(`/workallocation/case/task/${firstTask.case_id}`) &&
              response.status() === statusCode,
          );

          await taskListPage.clickTaskAction(
            taskListPage.taskActionGoTo,
            `my tasks go to ${statusCode} response`,
          );
