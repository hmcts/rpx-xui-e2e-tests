...
test.describe(
  `Task Unassign as ${userIdentifier}`,
  { tag: ["@integration", "@integration-manage-tasks"] },
  () => {
    test(`User can unassign one of their assigned tasks`, async ({
      taskListPage,
      page,
    }) => {
      ...
      await test.step("Submit unassign action and verify expected API response", async () => {
        const { submissionPromise: unassignResponsePromise } =
