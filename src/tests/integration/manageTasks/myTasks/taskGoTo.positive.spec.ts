...
test.describe(`@integration-bucket-3 Task Go To as ${userIdentifier}`, { tag: ['@integration', '@integration-manage-tasks'] }, () => {
  test(`User can use Go to task action from task list to view the task in case details`, async ({
    ...
    await page.route(`**/data/internal/cases/${firstTask.case_id}*`, async (route) => {
      await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(caseMockResponse) });
    });
    ...
    expect.soft(table[0]['Assigned to']).toContain(`${userRequestMockResponse[0].firstName} ${userRequestMockResponse[0].lastName}`);
