...
test.beforeEach(async ({ page }) => {
  ...
  taskListMockResponse = buildTaskListMock(
    160,
    userId?.toString() || "",
    myActionsList,
  );
...
test.describe(
  ...
  () => {
    test(`User sees service down when unassign endpoint returns 500`, async ({
      taskListPage,
      page,
    }) => {
      ...
      await test.step("Attempt unassign action and verify service down", async () => {
        ...
        await taskListPage.waitForManageButton(
          "my tasks unassign 500 response",
        );
        ...
        await taskListPage.clickTaskAction(
          taskListPage.taskActionUnassign,
          "my tasks unassign 500 response",
        );
        await expect(page).toHaveURL(
          new RegExp(`/work/${firstTask.id}/unclaim`),
        );

        const { submissionPromise: unassignFailureResponsePromise } =
          await setupUnassignSubmissionCapture(page, {
            taskId: firstTask.id,
            status: 500,
          });
        ...
        await taskListPage.submitActionAndWaitForRequest(
          (request) =>
            ...
            (request
              .url()
              .includes(`/workallocation/task/${firstTask.id}/unclaim`) ||
              request
                .url()
                .includes(`/workallocation/task/${firstTask.id}/assign`)),
        ...
        expect(["unclaim", "assign-null"]).toContain(
          unassignFailureResponse.mode,
        );
        ...
        await expect(
          page.getByRole("heading", {
            level: 1,
            name: SERVICE_DOWN_HEADING_TEXT,
          }),
        ).toBeVisible();
      });
    });

    test(`User returns to my tasks with warning when unassign endpoint returns 400`, async ({
      taskListPage,
      page,
    }) => {
      const firstTask = taskListMockResponse.tasks[0];
      ...
      await test.step("Attempt unassign action and verify redirect to my tasks with warning alert", async () => {
        ...
        await taskListPage.waitForManageButton(
          "my tasks unassign 400 response",
        );
        await taskListPage.openFirstManageActions(
          "my tasks unassign 400 response",
        );
        ...
        const { submissionPromise: badRequestResponsePromise } =
          await setupUnassignSubmissionCapture(page, {
            taskId: firstTask.id,
            status: 400,
          });
