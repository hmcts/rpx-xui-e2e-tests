...
test.describe(`Task List as ${userIdentifier}`, { tag: ['@integration', '@integration-manage-tasks'] }, () => {
  ...
  test(`User sees all priority type tasks with specific due dates`, async ({ taskListPage, page, tableUtils }) => {
    ...
    await test.step('Verify table shows deterministic priority tasks and due dates', async () => {
      ...
      const table = (await tableUtils.parseWorkAllocationTable(taskListPage.taskListTable)).filter(
        (row) => (row['Case name'] ?? '').trim().length > 0
      );
      expect(table.length).toBeGreaterThanOrEqual(4);
      const dataRows = taskListPage.taskListTable.locator('tbody tr:has(a[href*="/cases/case-details/"])');
      ...
      for (let i = 0; i < 4; i++) {
        ...
        const actualPriority = String(await dataRows.nth(i).locator('td').nth(7).innerText())
          .replace(/\s+/g, '')
          .toLowerCase();
