...
test.describe(
  ...
  () => {
    ...
    test(`User sees the expected message if there are no assigned tasks`, async ({
      taskListPage,
      page,
    }) => {
    ...
    test(`User sees all priority type tasks with specific due dates`, async ({
      taskListPage,
      page,
      tableUtils,
    }) => {
    ...
    test(`User can see all expected table elements with a large results set`, async ({
      taskListPage,
      page,
    }) => {
      await test.step("Setup route mock for deterministic task list", async () => {
        await setupTaskListMockRoutes(
          page,
          buildTaskListMock(1000, userId?.toString() || "", myActionsList),
        );
      });
