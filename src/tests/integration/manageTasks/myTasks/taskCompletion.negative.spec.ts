...
test.describe(
  ...
  () => {
    test(`User sees service down when complete endpoint returns 500`, async ({
      taskListPage,
      page,
    }) => {
      ...
      await test.step("Setup route mocks for list, action dependencies, and complete 500 response", async () => {
        ...
        await page.route(
          `**/workallocation/task/${firstTask.id}/complete*`,
          async (route) => {
      ...
      await test.step("Attempt complete action and verify service down", async () => {
        ...
        await taskListPage.openFirstManageActions(
          "my tasks complete 500 response",
        );
        await expect(taskListPage.taskActionMarkAsDone).toBeVisible();
        await taskListPage.clickTaskAction(
          taskListPage.taskActionMarkAsDone,
          "my tasks complete 500 response",
        );
        await expect(page).toHaveURL(
          new RegExp(`/work/${firstTask.id}/complete`),
        );
        ...
        const completeFailureResponsePromise = page.waitForResponse(
          (response) =>
            ...
            response
              .url()
              .includes(`/workallocation/task/${firstTask.id}/complete`) &&
            response.status() === 500,
        );
    ...
    test(`User returns to my tasks with warning when complete endpoint returns 400`, async ({
      taskListPage,
      page,
    }) => {
      const firstTask = taskListMockResponse.tasks[0];
      ...
      await test.step("Attempt complete action and verify redirect to my tasks with warning alert", async () => {
        ...
        await taskListPage.openFirstManageActions(
          "my tasks complete 400 response",
        );
        await expect(taskListPage.taskActionMarkAsDone).toBeVisible();
        await taskListPage.clickTaskAction(
          taskListPage.taskActionMarkAsDone,
