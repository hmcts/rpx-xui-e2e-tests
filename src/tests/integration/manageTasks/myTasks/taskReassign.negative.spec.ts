...
import { assertTaskReassignFailureOutcome } from '../../utils/taskReassignAssertions.js';
...
test.describe(
  `@integration-bucket-3 Task Reassign negative scenarios as ${userIdentifier}`,
    ...
    [
      ...
      test(title, async ({ taskListPage, page }) => {
        ...
        await test.step(`Attempt reassign action and verify behavior for ${statusCode}`, async () => {
          ...
          await assertTaskReassignFailureOutcome(page, taskListPage, statusCode);
        });
      });
    });
  }
