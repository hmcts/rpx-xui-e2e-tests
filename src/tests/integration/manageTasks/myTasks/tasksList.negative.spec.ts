...
test.describe(
  ...
  () => {
    ...
    test(`User ${userIdentifier} sees the no tasks message on my tasks, if the api times out`, async ({
      taskListPage,
      page,
    }) => {
      await test.step("Setup route mock for empty task list", async () => {
    ...
    test(`User ${userIdentifier} sees a service down message, if the api returns malformed json`, async ({
      ...
      await test.step("Verify the service down message is shown", async () => {
        expect(
          await taskListPage.exuiBodyComponent.serviceDownError.textContent(),
        ).toContain("Sorry, there is a problem with the service");
        expect(
          await taskListPage.exuiBodyComponent.serviceDownError.textContent(),
