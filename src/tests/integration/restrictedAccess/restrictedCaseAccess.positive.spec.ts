...
} from "../helpers/restrictedAccessMockRoutes.helper.js";
import {
...
test.describe(
  ...
  () => {
    test("shows both legal-ops and judicial users and resolves judicial users from ref-data", async ({
      ...
      const judicialLookupRequestPromise = page.waitForRequest(
      ...
      await expect(
        page.getByRole("heading", { level: 2, name: "Users with access" })
      ).toBeVisible();
      ...
      await expect(caseDetailsPage.exuiBodyComponent.tableHeaders.nth(2)).toHaveText(
        "Email address"
      );
      ...
      const expectedRows = [...DEFAULT_ROLE_ACCESS_USERS_OPS, ...DEFAULT_ROLE_ACCESS_USERS_JUDICIAL].map(
        (assignment) => {
      ...
      await expect
        .poll(async () => {
          ...
          return rows.length;
        }, { message: `restricted access table rows for ${testInfo.title}` })
    ...
    test("shows only legal-ops users and does not call judicial lookup when not needed", async ({
      ...
      const workerByIdamId = new Map(
        DEFAULT_CASEWORKERS_OPS.map((worker) => [worker.idamId, worker])
      );
    ...
    test("does not call judicial lookup when judicial users are already resolved by caseworker API", async ({
      ...
      const judicialCaseworkers = DEFAULT_CASEWORKERS.filter(
        (caseworker) => caseworker.roleCategory === "JUDICIAL"
      );
      ...
      await page.route("**/api/prd/judicial/searchJudicialUserByIdamId*", async () => {
        throw new Error(
          "Judicial lookup API should not be called when judicial users are resolved by caseworker API"
        );
      });
      ...
      const caseworkerByIdamId = new Map(
        judicialCaseworkers.map((worker) => [worker.idamId, worker])
      );
      const expectedRows = DEFAULT_ROLE_ACCESS_USERS_JUDICIAL.map((assignment) => {
