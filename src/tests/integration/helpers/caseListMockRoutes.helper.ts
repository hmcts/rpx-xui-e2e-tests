...
import { buildCaseListJurisdictionsMock, buildCaseListMock } from "../mocks/caseList.mock.js";
...
export async function setupCaseListMocks(
  ...
  await page
    .unroute("**/caseworkers/**/jurisdictions/**/case-types/**/work-basket-inputs*")
    .catch(() => undefined);
  ...
  await setupCaseListBaseRoutes(page, jurisdictions, workbasketInputs);
...
export async function waitForSearchCasesRequest(
  ...
  const request = await page.waitForRequest((candidate) => {
    ...
    if (
      requestUrl.pathname !== "/data/internal/searchCases" ||
      requestUrl.searchParams.get("page") !== String(expected.page)
    ) {
      return false;
    }
...
export async function expectCaseListSummary(
  caseListPage: CaseListPage,
  totalResults: number,
  pageNumber: number
): Promise<void> {
...
export async function expectPaginationState(
  ...
  if (options.previousVisible) {
    ...
    await expect(caseListPage.paginationPrevious).toBeHidden();
  }
  ...
  if (options.nextVisible !== undefined) {
    if (options.nextVisible) {
      ...
      await expect(caseListPage.paginationNext).toBeHidden();
    }
  }
