...
export async function setupHearingsMockRoutes(
  page: Page,
  ...
  const hearingById = new Map(
    (hearingScenarios ?? []).map((hearing) => [hearing.hearingId, hearing]),
  );
  ...
  const hearingsList = buildHearingsListMock(
    hearingScenarios,
    config.caseConfig,
  );
  const hearingValues = buildServiceHearingValuesMock(
    config.caseConfig,
    summaryHearing,
  );
  ...
  const hearingTypes = Array.from(
    new Set(
      [
        ...(hearingScenarios ?? []),
        ...(summaryHearing ? [summaryHearing] : []),
      ].map((hearing) => hearing.hearingType ?? "ABA5-ABC"),
    ),
  );
  ...
  await page.route(
    `**/data/internal/cases/${caseDetails.case_id}*`,
    async (route) => {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
    ...
    switch (requestPath(route)) {
      ...
      case "/api/hearings/getHearing":
        await route.fulfill({
          ...
          body: JSON.stringify(
            buildHearingRequestMock(
              (requestedHearingId(route)
                ? hearingById.get(String(requestedHearingId(route)))
                : undefined) ?? summaryHearing,
              config.caseConfig,
            ),
          ),
        });
  ...
  await page.route("**/api/prd/lov/getLovRefData*", async (route) => {
    ...
    const categoryKey =
      requestUrl.searchParams.get("categoryId") ??
      requestUrl.searchParams.get("categoryKey") ??
      undefined;
