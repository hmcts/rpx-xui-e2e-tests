...
export async function submitHeaderQuickSearch(
  ...
  const waitForCaseShell = async (timeoutMs = 15_000): Promise<void> => {
    ...
    while (Date.now() < deadline) {
      const headerInputVisible = await page.locator("#exuiCaseReferenceSearch").isVisible().catch(() => false);
      const primaryNavVisible = await page.locator(".hmcts-primary-navigation").first().isVisible().catch(() => false);
      if (headerInputVisible || primaryNavVisible) {
        return;
      }
      await sleep(250);
    }
    throw new Error(`Case shell did not become ready within ${timeoutMs}ms. URL=${page.url()}`);
  };
  ...
  const fallbackServicesCandidates = [page.locator("#servicesList"), page.getByLabel(/services/i)];
...
export async function submitGlobalSearchFromMenu(
  ...
  const searchLink = page
    .locator('.hmcts-primary-navigation__link[href="/search"], .hmcts-primary-navigation__link[href*="/search"]')
    .first();
