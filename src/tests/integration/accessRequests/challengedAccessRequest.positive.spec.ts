...
test.describe(
  ...
  () => {
    ...
    test("User sees the correct conditional inputs for each challenged access reason", async ({
      accessRequestPage,
      page
    }) => {
      await setupChallengedAccessMockRoutes(page);
      ...
      await expectChallengedAccessConditionalFields(accessRequestPage, {
        caseReferenceVisible: false,
        otherReasonVisible: false
      });

      await accessRequestPage.linkedCaseReasonRadio.check();
      await expectChallengedAccessConditionalFields(accessRequestPage, {
        caseReferenceVisible: true,
        otherReasonVisible: false
      });
    ...
    test("User can submit a challenged access request with case reference", async ({
      accessRequestPage,
      page
    }) => {
      await setupChallengedAccessMockRoutes(page);
      ...
      const requestPromise = page.waitForRequest(
        (request) =>
          request.method() === "POST" &&
          request.url().match(/\/api\/challenged-access-request(?:\?|$)/) !== null
      );
      ...
      expect(payload.roleRequest).toMatchObject({ process: "challenged-access" });
    ...
    test("User can submit a challenged access request with other reason", async ({
      accessRequestPage,
      page
    }) => {
      await setupChallengedAccessMockRoutes(page);
      ...
      await accessRequestPage.challengedOtherReasonInput.fill(
        "Urgent safeguarding review required before hearing."
      );
      ...
      expect(accessReasonDetails.otherReason).toBe(
        "Urgent safeguarding review required before hearing."
      );
