...
test.describe(
  `@integration-bucket-2 Review Specific Access Request negative paths as ${userIdentifier}`,
  { tag: ["@integration", "@integration-access-requests"] },
  () => {
    test("User cannot continue review flow without selecting an action", async ({
      accessRequestPage,
      page
    }) => {
    ...
    test("Review flow shows the service down page when task data cannot be loaded", async ({
      page
    }) => {
      await setupReviewSpecificAccessMockRoutes(page, {
        taskStatus: 500,
        taskBody: { message: "task load failed" }
      });
    ...
    test("Approve submit failures show the service down page", async ({
      accessRequestPage,
      page
    }) => {
      await setupReviewSpecificAccessMockRoutes(page, {
        approvalStatus: 500,
        approvalBody: { message: "approval failed" }
      });
      await page.goto(ACCESS_REQUEST_REVIEW_PATH, { waitUntil: "domcontentloaded" });
      await accessRequestPage.waitForReviewSpecificPage();
    ...
    test("Request more information submit failures show the service down page", async ({
      accessRequestPage,
      page
    }) => {
      await setupReviewSpecificAccessMockRoutes(page, {
