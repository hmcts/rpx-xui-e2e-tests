...
test.describe(`@integration-bucket-1 Case List as ${userIdentifier}`, { tag: ["@integration", "@integration-case-list"] }, () => {
  ...
  test(`User ${userIdentifier} can filter by multiple states and see the expected case list`, async ({
    ...
    const stateMockEntries = statesToVerify.map((state) => ({
      state,
      mock: buildCaseListMockForDefaultState(state)
    }));
    const stateMocks = new Map(
      stateMockEntries.map(({ state, mock }) => [state, mock])
    );
    ...
    for (const { state, mock: stateMock } of stateMockEntries) {
