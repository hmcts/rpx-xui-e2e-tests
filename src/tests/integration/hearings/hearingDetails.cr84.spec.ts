...
async function openHearingsTabForScenario(
  ...
  const url = caseDetailsUrl(
    options.caseConfig?.jurisdictionId,
    options.caseConfig?.caseTypeId,
  );
...
test.describe(
  `Hearings CR84 integration as ${userIdentifier}`,
  ...
  () => {
    test.describe("read-only CR84 summary routes", () => {
      ...
      for (const {
        ...
        roles,
        userLabel,
        expectListedSections,
      } of readOnlySummaryScenarios) {
        test(`Hearings - ${userLabel} can open ${name} hearing details and sees read-only summary content`, async ({
          ...
          await expect(
            hearingsTabPage.viewDetailsButton(scenario.hearingId),
          ).toBeVisible();
          await expect(
            hearingsTabPage.viewOrEditButton(scenario.hearingId),
          ).toHaveCount(0);
          await expect(
            hearingsTabPage.cancelButton(scenario.hearingId),
          ).toHaveCount(0);
          await expect(
            hearingsTabPage.addOrEditButton(scenario.hearingId),
          ).toHaveCount(0);
          ...
          await expect(
            hearingViewSummaryPage.summaryRow("Status"),
          ).toContainText(expectedStatus);
          await expect(
            hearingViewSummaryPage.summaryRow("Date request submitted"),
          ).toBeVisible();

          if (expectListedSections) {
            await expect(
              hearingViewSummaryPage.sectionHeading(
                "Listing information summary",
              ),
            ).toBeVisible();
            await expect(
              hearingViewSummaryPage.sectionHeading("Language requirements"),
            ).toBeVisible();
            await expect(
              hearingViewSummaryPage.sectionHeading("Additional facilities"),
            ).toBeHidden();
            await expect(
              hearingViewSummaryPage.sectionHeading("Hearing stage"),
            ).toBeHidden();
          }
        });
      }
    ...
    test.describe("manager summary routes for CR84 statuses", () => {
      ...
      for (const {
        ...
        test(`Hearings - hearing manager can open ${name} hearing details and sees amendable summary content`, async ({
          ...
          await expect(
            hearingViewSummaryPage.summaryRow(
              "Will additional security be required?",
            ),
          ).toContainText("No");
          await expect(
            hearingViewSummaryPage.summaryRow("Case internal name"),
          ).toContainText(HEARINGS_CASE_REFERENCE);
        });
      }
    });
      ...
      for (const actionMatrixScenario of actionMatrixScenarios) {
        test(`Hearings - ${actionMatrixScenario.name} sees the expected LISTED hearing actions`, async ({
          ...
          await hearingsTabPage.waitForReady(
            HEARINGS_LISTED_HEARING_ID,
            actionMatrixScenario.readyAction,
          );

          if (actionMatrixScenario.expectViewOrEdit) {
            await expect(
              hearingsTabPage.viewOrEditButton(HEARINGS_LISTED_HEARING_ID),
            ).toBeVisible();
          } else {
            await expect(
              hearingsTabPage.viewOrEditButton(HEARINGS_LISTED_HEARING_ID),
          ...
          if (actionMatrixScenario.expectViewDetails) {
            await expect(
              hearingsTabPage.viewDetailsButton(HEARINGS_LISTED_HEARING_ID),
            ).toBeVisible();
          } else {
            await expect(
              hearingsTabPage.viewDetailsButton(HEARINGS_LISTED_HEARING_ID),
          ...
          if (actionMatrixScenario.expectCancel) {
            await expect(
              hearingsTabPage.cancelButton(HEARINGS_LISTED_HEARING_ID),
            ).toBeVisible();
          } else {
            await expect(
              hearingsTabPage.cancelButton(HEARINGS_LISTED_HEARING_ID),
    ...
    test.describe("feature toggle routing", () => {
      ...
      test("Hearings - hearings-disabled case does not render the Hearings tab", async ({
        page,
      }) => {
        await applySessionCookies(page, userIdentifier);
        ...
        await page.goto(caseDetailsUrl("DIVORCE", "DIVORCE"), {
          waitUntil: "domcontentloaded",
        });
        await expect(page.getByRole("tab", { name: /hearings/i })).toHaveCount(
          0,
        );
      });
    });
    ...
    test("Hearings - user without hearing read rights cannot access LISTED hearing details entry points", async ({
      ...
      await page.goto("/hearings/view/hearing-view-summary", {
        waitUntil: "domcontentloaded",
      });
