...
import { HEARING_MANAGER_CR84_ON_USER, openHearingsTabForScenario } from '../helpers/index.js';
import {
...
import { assertListedActionMatrix, assertReadOnlySummarySections } from '../utils/hearingsAssertions.js';
...
test.describe(`@integration-bucket-4 Hearings CR84 integration as ${userIdentifier}`, { tag: ['@integration', '@integration-hearings'] }, () => {
  test.describe('read-only CR84 summary routes', () => {
    ...
    for (const { name, scenario, expectedStatus, roles, userLabel, expectListedSections } of readOnlySummaryScenarios) {
      test(`Hearings - ${userLabel} can open ${name} hearing details and sees read-only summary content`, async ({
        ...
        await assertReadOnlySummarySections(hearingViewSummaryPage, expectListedSections);
      });
    }
  });
  ...
  test.describe('LISTED role action matrix', () => {
    ...
    for (const actionMatrixScenario of actionMatrixScenarios) {
      test(`Hearings - ${actionMatrixScenario.name} sees the expected LISTED hearing actions`, async ({
        ...
        await assertListedActionMatrix(hearingsTabPage, HEARINGS_LISTED_HEARING_ID, {
          expectViewOrEdit: actionMatrixScenario.expectViewOrEdit,
          expectViewDetails: actionMatrixScenario.expectViewDetails,
          expectCancel: actionMatrixScenario.expectCancel
        });
      });
    }
  });
