...
const FORBIDDEN_KEYS = new Set([
  'authorization', 'body', 'command', 'cookie', 'cookies', 'form', 'header', 'headers', 'hostname',
  'osrelease', 'password', 'pid', 'platform', 'query', 'rawrequest', 'rawresponse', 'request', 'response',
  'stack', 'stderr', 'stdout',
]);
...
const repositoryPath = (value) => {
  ...
  if (workspaceIndex >= 0) return trimmed.slice(workspaceIndex + process.cwd().length + 1).replaceAll(path.sep, '/').slice(0, 500);
  const marker = ['/playwright_tests_new/', '/playwright_tests/', '/src/', '/test/'].find((part) => trimmed.includes(part));
  if (marker) return trimmed.slice(trimmed.indexOf(marker) + 1).replaceAll(path.sep, '/').slice(0, 500);
  const relative = path.relative(process.cwd(), trimmed);
  const candidate = !path.isAbsolute(trimmed) || (!relative.startsWith('..') && !path.isAbsolute(relative))
    ? (path.isAbsolute(trimmed) ? relative : trimmed)
...
const projectFailureData = (value) => {
  ...
  const execution = data.executionSignals && typeof data.executionSignals === 'object'
    ? {
...
const resolveLoadMetadata = () => {
  ...
  const cgroupMemoryLimit = rawMemoryLimit !== 'max' && parsedMemoryLimit > 0 && parsedMemoryLimit <= os.totalmem() * 2
    ? parsedMemoryLimit
    : undefined;
  return {
...
const getCpuTimes = () => os.cpus().reduce(
  (total, cpu) => ({
...
const createSystemSampler = (metadata) => {
  ...
  return (elapsedMs) => {
    ...
    const rawMemoryCurrent = metadata.memoryLimitSource === 'cgroup'
    ...
    const usedMemory = Number.isFinite(parsedMemoryCurrent) && parsedMemoryCurrent >= 0
      ? parsedMemoryCurrent
...
const buildSafeLoadProfile = (summary, samples) => {
  const timeline = downsample((Array.isArray(samples) ? samples : []).slice(0, MAX_RETAINED_LOAD_SAMPLES).map((sample, seq) => ({ sample, seq })))
    .flatMap(({ sample, seq }) => {
  ...
  return {
    ...
    sample_count: Math.min(MAX_RETAINED_LOAD_SAMPLES, integer(summary.sampleCount) ?? (Array.isArray(samples) ? samples.length : 0)),
...
class CiEvidenceReporter {
  constructor(options = {}) {
    ...
    this.outputFolder = options.outputFolder ?? this.env.PLAYWRIGHT_REPORT_FOLDER ?? this.env.PW_ODHIN_OUTPUT ?? 'test-results/odhin-report';
  ...
  onTestBegin(testCase, result) {
    ...
    const startedAt = result?.startTime instanceof Date && Number.isFinite(result.startTime.getTime())
      ? result.startTime
      : this.now();
  ...
  onEnd(result = {}) {
    ...
    for (const record of this.tests.values()) {
      ...
      const finalOutcome = record.playwright_outcome === 'expected'
        ? 'passed'
    ...
    const outcome = this.callbackCounts.globalErrors || ['failed', 'timedout', 'interrupted'].includes(playwrightStatus)
      ? 'FAILED'
      : counts.total === 0
      ? 'NOT_RUN'
    ...
    const shard = this.config?.shard && typeof this.config.shard === 'object'
