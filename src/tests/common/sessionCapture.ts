...
import { SessionCapturePage } from '../../page-objects/pages/exui/sessionCapture.po.js';
import { StorageStateCorruptedError, SessionCaptureError } from '../../utils/api/errors.js';
...
async function getIdamLoginErrorText(page: Page): Promise<string | null> {
  ...
  try {
    candidates = new SessionCapturePage(page).idamLoginErrorCandidates();
  } catch {
    return null;
  }
  for (const candidate of candidates) {
...
async function confirmAuthenticatedLogin(
  ...
  deps: {
    ...
    waitForShell?: (page: Page, userIdentifier: string, preferredSelector?: string, timeoutMs?: number) => Promise<unknown>;
    waitForAuthCookies?: (page: Page, timeoutMs: number) => Promise<boolean>;
...
async function sessionCaptureWith(identifiers: SessionIdentityInput[], deps: SessionCaptureDeps = {}) {
  ...
  const executeLoginAttemptFn = deps.executeLoginAttemptFn ?? executeLoginAttempt;
