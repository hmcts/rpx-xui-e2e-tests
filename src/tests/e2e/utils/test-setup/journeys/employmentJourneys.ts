...
import { uploadDocumentViaApi } from "../uploadDocumentViaApi.js";
    ...
    try {
      ...
      for (const eventLabel of preferredEventLabels) {
        try {
          ...
          const missingRequestedOption = message.includes(`option "${eventLabel}" not available`);
          const canTryAlternateEvent =
            missingRequestedOption && eventLabel !== preferredEventLabels.at(-1);
      ...
      if (isExternalDraftClaimFlow) {
        ...
        const resolvedAddressMode = await Promise.race([
          createCasePage.externalTriageAddressSelect
            .waitFor({ state: "visible" })
            .then(() => "address-select" as const),
          createCasePage.externalTriageAddressLine1Input
            .waitFor({ state: "visible" })
      ...
      await Promise.race([
        createCasePage.respondentAcasCertifcateSelectYes.waitFor({ state: "visible", timeout: 15_000 }),
        createCasePage.respondentOrganisation
          .waitFor({ state: "visible", timeout: 15_000 })
          .catch(() => undefined)
      ]).catch(() => undefined);
      const respondentOrganisationVisible = await createCasePage.respondentOrganisation
        .isVisible()
        .catch(() => false);
      ...
      const respondentCompanyNameVisible = await createCasePage.respondentCompanyNameInput
        .isVisible()
        .catch(() => false);
      ...
      if (await createCasePage.sameAsClaimantWorkAddressYes.isVisible().catch(() => false)) {
        ...
        logger.info(
          "Employment create: work address confirmation not shown; advancing with current wizard state",
          {
            attempt,
            url: createCasePage.page.url()
          }
...
async function ensureEmploymentDraftClaimantRepresentationAnswered(createCasePage: CreateCasePage): Promise<void> {
  ...
  const claimantRepresentationGroup = createCasePage.page.getByRole("group", {
