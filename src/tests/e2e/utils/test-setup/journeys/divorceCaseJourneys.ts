...
import { isTransientWorkflowFailure } from "../../transient-failure.utils.js";
...
export async function createDivorceCaseFlag(
  ...
  await createCasePage.clickSubmitAndWait("after submitting divorce case flags", { timeoutMs: 60_000 });
...
export async function createDivorceCasePoC(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try {
      ...
      await createCasePage.clickSubmitAndWait("after submitting divorce PoC case", { timeoutMs: 60_000 });
