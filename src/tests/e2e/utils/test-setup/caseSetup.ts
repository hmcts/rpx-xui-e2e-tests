...
async function resolveApiIdsFromAggregatedJurisdictions({
  ...
  const requestedCaseType = normalizeLookupValue(request.caseType);
...
async function createCaseViaDirectCcdApi(request: SetupCaseRequest): Promise<string | undefined> {
  ...
  const effectiveTimeoutMs =
    Number.isFinite(timeoutMs) && timeoutMs > 0 ? timeoutMs : DEFAULT_CASE_SETUP_TIMEOUT_MS;
  ...
  while (!(tokenResponse.status() >= 200 && tokenResponse.status() < 300)) {
    ...
    await waitForRetryInterval(Math.min(tokenRetryIntervalMs, Math.max(remainingMs, 0)));
  ...
  for (let attempt = 1; attempt < createRetryAttempts; attempt += 1) {
    ...
    await waitForRetryInterval(Math.min(createRetryIntervalMs, Math.max(remainingMs, 0)));
  ...
  await request.page.goto(
    `/cases/case-details/${resolvedIds.jurisdictionId}/${resolvedIds.caseTypeId}/${caseNumber}`
  );
