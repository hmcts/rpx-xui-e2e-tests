...
async function runBrowserFetchText(
  ...
  return page.evaluate(async (request) => {
    ...
    const binary = Uint8Array.from(atob(request.multipart.fileContentBase64), (character) =>
...
async function waitForXsrfToken(page: Page, baseUrl: string): Promise<string> {
  ...
  throw new Error(
    `Document upload setup failed: XSRF-TOKEN cookie was not available within ${XSRF_COOKIE_WAIT_TIMEOUT_MS}ms ` +
      `(url=${currentPageUrl(page)}, cookies=${cookieNames.join(",") || "none"})`
  );
}
