...
export function shouldPreferManageOrgInvitePrimaryForPrincipal(
  ...
  const manageOrgPrincipalRoles = MANAGE_ORG_ASSIGNMENT_PRINCIPAL_ROLES as ReadonlySet<string>;
  return (assignmentUserRoles.roles ?? []).some((role) => manageOrgPrincipalRoles.has(role));
}
