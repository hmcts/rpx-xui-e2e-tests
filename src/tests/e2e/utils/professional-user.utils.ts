...
const resolveBearerTokenPrincipalUsername = (token: string): string | undefined =>
  resolveTokenPrincipalUsername(decodeJwtPayload(token));
...
export class ProfessionalUserUtils {
  ...
  private async inviteUserViaManageOrgApi(params: {
    ...
    logger.info('Resolved assignment principal roles for manage-org bearer invite.', {
      assignmentPrincipalUsername: resolveBearerTokenPrincipalUsername(assignmentBearerToken) ?? 'unknown',
