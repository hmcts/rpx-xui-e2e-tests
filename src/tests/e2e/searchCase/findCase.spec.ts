...
if (userUtils.hasUserCredentials("FPL_GLOBAL_SEARCH")) {
  test.describe("FPL global search user - find case", () => {
    let availableCaseReference = "";

    test.use({
      storageState: resolveUiStoragePathForUser("FPL_GLOBAL_SEARCH"),
    });

    test.beforeAll(async () => {
      await ensureSessionCookies("FPL_GLOBAL_SEARCH", { strict: true });
    });
    ...
    test("Find case using Public Law jurisdiction", async ({
      ...
      await test.step("Verify that case searched for appears under 'Your cases'", async () => {
        await findCasePage.searchResultsDataTable.waitFor({
          state: "visible",
        });
        const searchTable = await tableUtils.mapExuiTable(
...
if (userUtils.hasUserCredentials("SOLICITOR")) {
  test.describe("Solicitor navigation to Find case (top-right)", () => {
    test.use({ storageState: resolveUiStoragePathForUser("SOLICITOR") });

    test.beforeAll(async () => {
      await ensureSessionCookies("SOLICITOR", { strict: true });
    });
