...
import { withXsrf, expectStatus, StatusSets, withRetry } from './utils/apiTestUtils.js';
...
test.describe('CCD endpoints', () => {
  test('lists jurisdictions for current user', async ({ apiClient }) => {
    ...
    const actualNames = (data ?? []).map((entry: any) => entry?.name).filter(Boolean);
    expectedNames.forEach((name) => {
      expect(actualNames).toContain(name);
    });

    data.forEach((jurisdiction: any) => {
