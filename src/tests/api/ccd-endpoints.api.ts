...
test.describe("CCD endpoints", () => {
  ...
  const jurisdictions = (testConfig.jurisdictions[
    testConfig.testEnv as "aat" | "demo"
  ...
  for (const jurisdiction of jurisdictions) {
    ...
    for (const caseTypeId of uniqueCaseTypes) {
      test(`work-basket inputs available for ${caseTypeId}`, async ({
        apiClient,
      }) => {
...
async function assertJurisdictionsForUser(apiClient: ApiClient): Promise<void> {
  ...
  }>("api/user/details", { throwOnError: false });
  ...
  const jurisdictionsData = Array.isArray(response.data)
  ...
  const expectedNames =
    testConfig.jurisdictionNames[testConfig.testEnv as "aat" | "demo"] ?? [];
  const actualNames = jurisdictionsData
    .map((entry) => entry?.name)
    .filter(Boolean);
  if (expectedNames.length) {
    const overlap = actualNames.filter((name) =>
...
async function assertWorkBasketInputs(
  apiClient: ApiClient,
  caseTypeId: string,
): Promise<void> {
  ...
  const inputs = Array.isArray(data?.workbasketInputs)
    ? (data?.workbasketInputs ?? [])
    : [];
