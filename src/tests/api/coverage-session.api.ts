...
test.describe(
  "Session and cookie utilities coverage",
  { tag: "@svc-internal" },
  () => {
    test("isSessionFresh returns false when stat fails", () => {
      ...
      expect(
        isSessionFresh("session.json", 1000, { fs: fsStub, now: () => 1000 }),
      ).toBe(false);
    });
    ...
    test("CookieUtils writes and updates session files", async () => {
      ...
      const tmpDir = await fsp.mkdtemp(
        path.join(process.cwd(), "test-results", "cookie-utils-"),
      );
      ...
      const added = updated.cookies.find(
        (cookie: any) =>
          cookie.name === "hmcts-exui-cookies-user-1-mc-accepted",
      );
      expect(added).toBeDefined();
      expect(added.value).toBe("true");

      const noUserPath = path.join(tmpDir, "no-user.json");
      cookieUtils.writeManageCasesSession(noUserPath, [
        baseCookie("other", "1"),
      ]);
      ...
      cookieUtils.writeManageCasesSession(withUserPath, [
        baseCookie("__userid__", "user-2"),
      ]);
      const withUserState = JSON.parse(
        await fsp.readFile(withUserPath, "utf8"),
      );
      const withUserCookie = withUserState.cookies.find(
        (cookie: any) =>
          cookie.name === "hmcts-exui-cookies-user-2-mc-accepted",
      );
      ...
      cookieUtils.writeManageCasesSession(nestedPath, [
        baseCookie("__userid__", "user-3"),
      ]);
      const nestedState = JSON.parse(await fsp.readFile(nestedPath, "utf8"));
      expect(
        nestedState.cookies.find(
          (cookie: any) =>
            cookie.name === "hmcts-exui-cookies-user-3-mc-accepted",
        ),
      ).toBeDefined();
      ...
      await fsp.writeFile(
        noUserAnalyticsPath,
        JSON.stringify({ cookies: [baseCookie("other", "1")] }),
        "utf8",
      );
      await cookieUtils.addManageCasesAnalyticsCookie(noUserAnalyticsPath);
      const noUserAnalytics = JSON.parse(
        await fsp.readFile(noUserAnalyticsPath, "utf8"),
      );
    ...
    test("CookieUtils surfaces errors when session data is invalid", async () => {
      const cookieUtils = new CookieUtils();
      const tmpDir = await fsp.mkdtemp(
        path.join(process.cwd(), "test-results", "cookie-utils-error-"),
      );
      const badPath = path.join(tmpDir, "bad.json");
      await fsp.writeFile(badPath, "{bad-json", "utf8");
      await expect(
        cookieUtils.addManageCasesAnalyticsCookie(badPath),
      ).rejects.toThrow("Failed to read or write session data");
      ...
      expect(() =>
        cookieUtilsFailing.writeManageCasesSession(
          path.join(tmpDir, "fail.json"),
          [],
        ),
      ).toThrow("Failed to write session file");
    });

    test("sessionCapture helpers handle fresh, stale, and missing sessions", async () => {
      const tmpDir = await fsp.mkdtemp(
        path.join(process.cwd(), "test-results", "session-utils-"),
      );
      ...
      try {
        ...
        await fsp.writeFile(
          storagePath,
          JSON.stringify({ cookies: [baseCookie("a", "b")] }),
          "utf8",
        );
        ...
        expect(() => loadSessionCookies("IAC_CaseOfficer_R1")).toThrow(
          "Storage file corrupted",
        );

        await fsp.rm(storagePath, { force: true });
        expect(() => loadSessionCookies("IAC_CaseOfficer_R1")).toThrow(
          "Failed parsing storage file",
        );
      } finally {
    ...
    test("persistSession writes cookies and surfaces errors", async () => {
      const tmpDir = await fsp.mkdtemp(
        path.join(process.cwd(), "test-results", "session-persist-"),
      );
      const sessionPath = path.join(tmpDir, "session.json");
      const ctx = {
      ...
      await sessionCaptureTest.persistSession(
        sessionPath,
        [baseCookie("a", "b")],
        ctx as any,
        "user",
