...
test.describe(
  "Evidence Manager & Documents",
  { tag: "@svc-evidence-manager" },
  () => {
    test.beforeAll(async () => {
      sharedDocId = await resolveSharedDocId(
        configuredDocId,
        uploadSyntheticDoc,
      );
      if (!sharedDocId) {
    ...
    test("returns document binary with XSRF", async ({ apiClient }) => {
      await withXsrf("solicitor", async (headers) => {
        const res = await apiClient.get<ArrayBuffer | string>(
          `documents/${sharedDocId}/binary`,
          {
    ...
    test("rejects unauthenticated binary fetch", async ({
      anonymousClient,
    }) => {
      const res = await anonymousClient.get(`documents/${sharedDocId}/binary`, {
        throwOnError: false,
      });
      expectStatus(res.status, [401, 403]);
    });

    test("annotations metadata guarded by session", async ({
      apiClient,
      anonymousClient,
    }) => {
      const anon = await anonymousClient.get(
        `em-anno/metadata/${sharedDocId}`,
        { throwOnError: false },
      );
    ...
    test("creates and deletes annotation with valid XSRF", async ({
      apiClient,
    }) => {
      await withXsrf("solicitor", async (headers) => {
        const annotation = await buildAnnotation(
          apiClient,
          headers,
          sharedDocId,
        );
    ...
    test("bookmarks lifecycle", async ({ apiClient }) => {
      await withXsrf("solicitor", async (headers) => {
        const listRes = await apiClient.get<Array<any>>(
          `em-anno/${sharedDocId}/bookmarks`,
          {
            headers,
            throwOnError: false,
          },
    ...
    test("rejects bookmark creation with invalid XSRF token", async ({
      apiClient,
    }) => {
      const bookmark = await buildBookmark(apiClient, sharedDocId);
    ...
    test("rejects bookmark mutation with invalid payload", async ({
      apiClient,
    }) => {
      await withXsrf("solicitor", async (headers) => {
        const res = await apiClient.put("em-anno/bookmarks", {
    ...
    test("returns guarded status for invalid document id on delete", async ({
      apiClient,
    }) => {
      await withXsrf("solicitor", async (headers) => {
        const bogusId = uuid();
    ...
    test("rejects bookmark delete for missing document", async ({
      apiClient,
    }) => {
      await withXsrf("solicitor", async (headers) => {
...
test.describe(
  "Evidence Manager helper coverage",
  { tag: "@svc-evidence-manager" },
  () => {
    test("resolveConfiguredDocId and resolveSharedDocId handle overrides", async () => {
      ...
      expect(resolveConfiguredDocId(undefined, "fallback-doc")).toBe(
        "fallback-doc",
      );

      const configured = await resolveSharedDocId(
        "configured-doc",
        async () => "uploaded-doc",
      );
      expect(configured).toBe("configured-doc");

      const uploaded = await resolveSharedDocId(
        undefined,
        async () => "uploaded-doc",
      );
    ...
    test("annotation helpers cover ids and responses", () => {
      const data = {
        annotations: [
          { id: "anno-1", documentId: "doc-1", annotationSetId: "set-1" },
        ],
      };
    ...
    test("resolveAnnotationSetId handles valid, fallback, and errors", async () => {
      ...
      expect(await resolveAnnotationSetId(apiClient, {}, sharedDocId)).toBe(
        "set-1",
      );

      const apiClientMissing = { get: async () => ({ data: { id: 123 } }) };
      const fallback = await resolveAnnotationSetId(
        apiClientMissing,
        {},
        sharedDocId,
      );
      ...
      const errorFallback = await resolveAnnotationSetId(
        apiClientError,
        {},
        sharedDocId,
      );
    ...
    test("buildAnnotation and buildBookmark return expected payloads", async () => {
      ...
      const bookmarkClient = {
        get: async () => ({ data: { userInfo: { uid: "user-1" } } }),
      };
    ...
    test("resolveUploadedDocId handles possible document ids", () => {
      expect(
        resolveUploadedDocId({ documents: [{ originalDocumentId: "orig" }] }),
      ).toBe("orig");
      expect(resolveUploadedDocId({ documents: [{ documentId: "doc" }] })).toBe(
        "doc",
      );
