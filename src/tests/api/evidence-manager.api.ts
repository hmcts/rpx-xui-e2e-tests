...
import { ensureStorageState, getStoredCookie } from './auth.js';
import { test, expect } from './fixtures.js';
import { config } from './config.js';
import { EM_DOC_ID } from './data/testIds.js';
import { expectAnnotationShape, expectBookmarkShape } from './utils/assertions.js';
import { expectStatus, StatusSets, withXsrf } from './utils/apiTestUtils.js';
import { AnnotationPayload, BookmarkPayload } from './utils/types.js';
...
test.describe('Evidence Manager & Documents', () => {
  ...
  test('returns document binary with XSRF', async ({ apiClient }) => {
    await withXsrf('solicitor', async (headers) => {
      const res = await apiClient.get<unknown>(`documents/${sharedDocId}/binary`, {
      ...
      if (res.status === 200) {
        expect((res.data as ArrayBuffer | undefined)?.byteLength ?? 0).toBeGreaterThan(0);
      }
    });
  });
  ...
  test('bookmarks lifecycle', async ({ apiClient }) => {
    await withXsrf('solicitor', async (headers) => {
      const listRes = await apiClient.get<unknown>(`em-anno/${sharedDocId}/bookmarks`, {
        headers,
        throwOnError: false
      });
