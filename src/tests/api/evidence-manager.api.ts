...
async function assertDocumentBinary(
  apiClient: ApiClient,
...
async function assertAnnotationLifecycle(
  apiClient: ApiClient,
  headers: Record<string, string>,
): Promise<void> {
  ...
  if (
    ...
    if (created) {
      ...
      const deleteRes = await apiClient.delete(
        `em-anno/annotations/${created.id}`,
        {
          headers,
          throwOnError: false,
        },
...
async function assertBookmarkCreated(
  apiClient: ApiClient,
  headers: Record<string, string>,
): Promise<void> {
  const body = buildBookmarkPayload();
  const res = await apiClient.post<{ bookmarks?: BookmarkPayload[] }>(
    "documents/bookmarks",
    {
      headers,
      data: body,
...
function assertEditedDocumentPath(res: {
  status: number;
  data: unknown;
}): void {
  if (
...
async function uploadSyntheticDoc(): Promise<string> {
  ...
  try {
    const content = Buffer.from(
      "<html><body><h1>Playwright synthetic doc</h1></body></html>",
      "utf8",
    );
