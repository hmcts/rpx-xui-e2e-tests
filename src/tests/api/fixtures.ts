...
import { ensureStorageState, getStoredCookie, hasApiUser, type ApiUserRole } from './auth.js';
import { buildAuthHeaders } from '../../utils/api/auth.js';
import CONFIG from '../../../config/configManager.js';

const baseUrl = stripTrailingSlash(config.baseUrl);
const apiTimeoutMs = (config as any)?.test?.apiRequestTimeout ?? CONFIG.test.apiRequestTimeout ?? 10000;
...
async function createNodeApiClient(
  ...
  const hasConfiguredUser = role !== 'anonymous' && hasApiUser(role as ApiUserRole);
  const tokenHeaders = role === 'anonymous' ? {} : await buildAuthHeaders();
  const authHeaders = hasConfiguredUser
    ? Object.fromEntries(
        Object.entries(tokenHeaders).filter(([key]) => key.toLowerCase() === 'serviceauthorization')
      )
    : tokenHeaders;
  const hasBearer = Boolean(authHeaders.Authorization);
  const storageState =
    role === 'anonymous'
      ? undefined
      : hasConfiguredUser
  ...
  if (!hasBearer && storageState && role !== 'anonymous' && shouldAutoInjectXsrf()) {
