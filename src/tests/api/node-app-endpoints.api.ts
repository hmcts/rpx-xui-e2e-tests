...
import { ensureStorageState } from './auth.js';
import { test, expect, buildApiAttachment } from './fixtures.js';
import { expectStatus, StatusSets } from './utils/apiTestUtils.js';

import nodeAppDataModels from '../../test_codecept/dataModels/nodeApp.js';
...
test.describe('Node app endpoints', () => {
  ...
  test('returns enriched user details for solicitor session', async ({ apiClient }, testInfo) => {
    ...
    if (Array.isArray(userInfo.roles) && userInfo.roles.length > 0) {
      expect(userInfo.roles[0]).toEqual(expect.any(String));
    }
    const expected = nodeAppDataModels.getUserDetails_oidc();
    if (response.data?.sessionTimeout) {
      expect(response.data.sessionTimeout).toEqual(
        expect.objectContaining({
    ...
    const expectedRoleAssignments = Array.isArray(expected.roleAssignmentInfo) ? expected.roleAssignmentInfo : [];
    const firstExpectedRole = expectedRoleAssignments.find(
    ...
    if (
      ...
      const expectedRoleKeys = Object.keys(firstExpectedRole);
  ...
  test('stale session cookie returns guarded status', async () => {
    ...
    const ctx = await request.newContext({
      baseURL: testConfig.baseUrl.replace(/\/+$/, ''),
      ignoreHTTPSErrors: true,
      storageState: expiredCookies.length ? { cookies: expiredCookies, origins: [] } : undefined
    });
