...
import { __test__ as authTest, type ApiUserRole } from "../../fixtures/api-auth";
...
test.describe("Auth helper coverage - basic utilities", () => {
  ...
  test("getCredentials returns configured users and errors on unknown roles", () => {
    ...
    expect(() => authTest.getCredentials("unknown" as ApiUserRole)).toThrow("No credentials configured");
  });
});
