...
test.describe("Role access / AM", () => {
  ...
  test("roles/access-get responds", async ({ apiClient }) => {
    const res = await withRetry(
      () =>
        apiClient.post<RoleAssignmentContainer>(
          "api/role-access/roles/access-get",
          {
  ...
  test("roles/getJudicialUsers responds", async ({ apiClient }) => {
    const res = await apiClient.post("api/role-access/roles/getJudicialUsers", {
      data: {},
      throwOnError: false,
    });
...
test.describe("Case flags", () => {
  ...
  test("rejects unauthenticated call to all-judicial-users", async ({
    anonymousClient,
  }) => {
    const res = await anonymousClient.get(
      "api/role-access/roles/all-judicial-users",
      {
        throwOnError: false,
        headers: { experimental: "true" },
      },
...
function assertGlobalSearchServices(response: {
  status: number;
  data: unknown;
}): void {
  if (
...
function assertGlobalSearchResults(response: {
  status: number;
  data: unknown;
}): void {
...
function assertSearchCasesResponse(response: {
  status: number;
  data: any;
}): void {
  if (response.status === 200 && response.data) {
...
function assertSupportedJurisdictions(response: {
  status: number;
  data: unknown;
}): void {
  if (
...
function assertLocationsResponse(response: {
  status: number;
  data: unknown;
}): void {
  if (
...
function assertMyAccessCount(response: {
  status: number;
  data?: unknown;
}): void {
...
async function safeGetMyAccessCount(apiClient: {
  get: (
    path: string,
...
function assertAccessGetResponse(response: {
  status: number;
  data: any;
}): void {
  if (response.status === 200) {
    if (Array.isArray(response.data) && response.data.length > 0) {
      ...
      Array.isArray(
        (response.data as RoleAssignmentContainer)?.roleAssignmentResponse,
      ) &&
      (response.data as RoleAssignmentContainer).roleAssignmentResponse!
        .length > 0
    ) {
      expectRoleAssignmentShape(
        (response.data as RoleAssignmentContainer)
          .roleAssignmentResponse![0] as any,
      );
    } else {
      expect(response.data).toEqual(expect.anything());
    }
...
function assertValidRolesResponse(response: {
  status: number;
  data: unknown;
}): void {
  if (
