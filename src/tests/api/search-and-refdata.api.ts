...
test.describe("Global search", { tag: "@svc-global-search" }, () => {
  test("lists available services", async ({ apiClient }) => {
    const response = await withRetry(
      () =>
        apiClient.get<Array<{ serviceId: string; serviceName: string }>>(
  ...
  test("searchCases proxy responds or guards", async ({ apiClient }) => {
    const response = await withRetry(
      () =>
        apiClient.post<{ total?: number; cases?: unknown[] }>(
          "data/internal/searchCases?ctid=xuiTestCaseType",
          {
            data: {
              size: 1,
              from: 0,
              sort: [],
              native_es_query: { match_all: {} },
            },
            throwOnError: false,
          },
        ),
...
test.describe(
  "Ref data and supported jurisdictions",
  { tag: "@svc-ref-data" },
  () => {
    ...
    test("locations endpoint returns list or guarded status", async ({
      apiClient,
    }) => {
      await withXsrf("solicitor", async (headers) => {
        const res = await apiClient.get<Array<{ epimms_id?: string }>>(
          "api/locations",
          {
            headers,
            throwOnError: false,
    ...
    test("staff-ref-data endpoint responds", async ({ apiClient }) => {
      const res = await apiClient.post<{
        staff?: Array<{ known_as?: string; email_id?: string }>;
      }>("api/staff-ref-data/search", {
...
test.describe("Role access / AM", { tag: "@svc-role-assignment" }, () => {
  ...
  test.beforeAll(async ({ apiClient }) => {
    ...
    roleAccessCaseId = ensureCcdCaseReference(
      resolveRoleAccessCaseId(roleAccessCaseId),
  ...
  test("rejects unauthenticated role access calls", async ({
    ...
    const res = await anonymousClient.post(
  ...
  test("rejects role access mutation with invalid CSRF token", async ({
    apiClient,
  }) => {
    const payload = buildRoleAllocationRequest(
      resolveRoleAccessCaseId(roleAccessCaseId),
    );
  ...
  test("get-my-access-new-count", async ({ apiClient }) => {
    const res = await withRetry(
      () =>
        apiClient.get<{ count?: number } | number>(
  ...
  test("roles/access-get responds", async ({ apiClient }, testInfo) => {
    const payload = buildCaseIdListPayload(
      resolveRoleAccessCaseId(roleAccessCaseId),
    );
    const res = await withRetry(
      () =>
  ...
  test("roles/access-get-by-caseId responds with roles when present", async ({
    apiClient,
  }, testInfo) => {
    const payload = buildCaseIdPayload(
      resolveRoleAccessCaseId(roleAccessCaseId),
    );
    const res = await postWaWithDiagnostics<RoleAssignmentContainer>(
      apiClient,
      {
  ...
  test("specific-access approval flow guarded with XSRF", async ({
    apiClient,
  }, testInfo) => {
    const payload = buildSpecificAccessApprovalRequest(
      resolveRoleAccessCaseId(roleAccessCaseId),
    );
    await withXsrf("solicitor", async (headers) => {
  ...
  test("allocate-role/confirm responds with XSRF header", async ({
    apiClient,
  }, testInfo) => {
    const payload = buildRoleAllocationRequest(
      resolveRoleAccessCaseId(roleAccessCaseId),
  ...
  test("allocate-role/reallocate responds with XSRF header", async ({
    apiClient,
  }, testInfo) => {
    const payload = buildRoleAllocationRequest(
      resolveRoleAccessCaseId(roleAccessCaseId),
  ...
  test("allocate-role/delete responds with XSRF header", async ({
    apiClient,
  }) => {
    await withXsrf("solicitor", async (headers) => {
      const res = await apiClient.post<RoleAssignmentContainer>(
  ...
  test("exclusions/confirm responds with XSRF header", async ({
    apiClient,
  }) => {
    await withXsrf("solicitor", async (headers) => {
      const res = await apiClient.post<RoleAssignmentContainer>(
  ...
  test("roles/post responds with XSRF header", async ({
    apiClient,
  }, testInfo) => {
    const payload = buildCaseIdPayload(
      resolveRoleAccessCaseId(roleAccessCaseId),
  ...
  test("role access confirm rejects case officer without solicitor role", async ({
    apiClientFor,
  }, testInfo) => {
    if (!hasCaseOfficer) {
    ...
    try {
      ...
      const res = await client.post("api/role-access/allocate-role/confirm", {
        data: buildRoleAllocationRequest(
          resolveRoleAccessCaseId(roleAccessCaseId),
        ),
        throwOnError: false,
      });
      ...
      if (error instanceof AuthenticationError) {
        ...
        test.skip(
          true,
          `caseOfficer_r1 cannot authenticate in ${config.testEnv}`,
        );
        return;
      }
      throw error;
  ...
  test("roles/manageLabellingRoleAssignment responds", async ({
    apiClient,
  }, testInfo) => {
    const caseId = ensureCcdCaseReference(
      resolveRoleAccessCaseId(roleAccessCaseId),
      "manage-labelling caseId",
    );
    const res = await withRetry(
      () =>
...
test.describe(
  ...
  () => {
    ...
    test("assertStaffRefDataResponse covers staff payloads", () => {
      assertStaffRefDataResponse(200, {
        staff: [{ known_as: "Name", email_id: "a@b.com" }],
      });
    ...
    test("assertRoleAccessGetResponse handles array and container shapes", () => {
      assertRoleAccessGetResponse(200, [
        { roleCategory: "LEGAL", roleName: "role" },
      ]);
      assertRoleAccessGetResponse(200, {
        roleAssignmentResponse: [{ roleCategory: "LEGAL", roleName: "role" }],
      });
      assertRoleAccessGetResponse(200, []);
    ...
    test("assertRoleAccessByCaseIdResponse handles roleAssignmentResponse", () => {
      assertRoleAccessByCaseIdResponse(200, {
        roleAssignmentResponse: [{ roleCategory: "LEGAL", roleName: "role" }],
      });
    ...
    test("assertManageLabellingResponse handles guarded and success statuses", () => {
      assertManageLabellingResponse(200, [
        { roleCategory: "LEGAL", roleName: "role" },
      ]);
    ...
    test("assertRoleAssignmentsIfPresent handles success and empty arrays", () => {
      ...
      assertRoleAssignmentsIfPresent(201, [
        { roleCategory: "LEGAL", roleName: "role" },
      ]);
      assertRoleAssignmentsIfPresent(200, []);
    ...
    test("buildExpiredCookies handles missing cookies", () => {
      expect(
        buildExpiredCookies({
          cookies: [{ name: "c", value: "1", expires: 0 }],
        }),
      ).toHaveLength(1);
