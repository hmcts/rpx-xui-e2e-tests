...
test.describe(
  "Configuration resolution coverage",
  { tag: "@svc-internal" },
  () => {
    test("apiTestConfig helpers resolve env values", () => {
      expect(apiTestConfigTest.resolveBaseUrl(undefined)).toBe(
        "https://manage-case.aat.platform.hmcts.net/",
      );
      expect(apiTestConfigTest.resolveBaseUrl("https://example.test")).toBe(
        "https://example.test",
      );
    ...
    test("appTestConfig helpers resolve preview and test env", () => {
      ...
      expect(preview).toEqual(
        expect.objectContaining({ demoUrl: "https://demo.example" }),
      );
      expect(
        appTestConfigTest.resolvePreviewConfig(
          [],
          "https://preview.example/path",
        ),
      ).toBeUndefined();
      ...
      expect(
        appTestConfigTest.applyPreviewConfig(
    ...
    test("config.utils resolves env vars and URLs", async () => {
      expect(
        configUtilsTest.resolveUrl("https://override", "https://fallback"),
      ).toBe("https://override");
      expect(configUtilsTest.resolveUrl(undefined, "https://fallback")).toBe(
        "https://fallback",
      );
      ...
      await withEnv({ CONFIG_TEST_VAR: undefined }, () => {
        expect(() => configUtilsTest.getEnvVar("CONFIG_TEST_VAR")).toThrow(
          "CONFIG_TEST_VAR",
        );
      });
