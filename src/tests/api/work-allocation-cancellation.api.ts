...
test.describe("Work allocation cancellation API coverage", () => {
  test.beforeAll(async ({ apiClient }) => {
    const firstTask = await fetchFirstTask(
      apiClient,
      undefined,
      ["assigned", "unassigned"],
      "AllWork",
  ...
  test("POST /workallocation/task/:id/cancel accepts the UI manual cancellation payload", async ({
    ...
    const cancelRequestLogs = apiLogs.slice(startLogIndex).filter((entry) => {
      return (
        entry.method === "POST" &&
        entry.url.includes(`/workallocation/task/${taskId}/cancel`)
      );
    });
    ...
    expect(
      latestCancelCall,
      "Expected at least one cancel request log entry",
    ).toBeTruthy();
    if (!latestCancelCall) {
      throw new Error(
        "No cancel request logs captured for cancellation coverage.",
      );
    }
      ...
      body: JSON.stringify(
        {
          ...
          actual: {
            url: actualUrl,
            includesCompletionProcess: actualUrl.includes(
              "completion_process=",
            ),
          },
        },
  ...
  for (const processCase of cancellationProcessMatrix) {
    test(`POST /workallocation/task/:id/cancel tolerates optional ${processCase.label} query`, async ({
      ...
      await testInfo.attach(
        `EXUI-3662-optional-query-${processCase.value}-expected-vs-actual.json`,
        {
          body: JSON.stringify(
            {
              expectation: {
