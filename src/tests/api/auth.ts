...
export function hasApiUser(role: ApiUserRole): boolean {
  const envUsers = config.users[config.testEnv as keyof typeof config.users];
  return Boolean(envUsers && (envUsers as Record<string, unknown>)[role]);
}
...
export async function getStoredCookie(role: ApiUserRole, cookieName: string): Promise<string | undefined> {
  ...
  const cookie = Array.isArray(state.cookies)
    ? state.cookies.find((c: { name?: string; value?: unknown }) => c.name === cookieName)
    : undefined;
...
async function tryTokenBootstrap(
  ...
  const clientSecret = process.env.IDAM_CLIENT_SECRET ?? process.env.IDAM_SECRET;
  ...
  try {
    ...
    const serviceToken = await serviceAuthUtils.retrieveToken({
      microservice,
...
// Expose small helpers for unit coverage.
export const __test__ = { extractCsrf, stripTrailingSlash, getCredentials, isTokenBootstrapEnabled };
...
async function tryReadState(
  storagePath: string
): Promise<{ cookies?: Array<{ name?: string; value?: unknown }> } | undefined> {
  try {
