...
const loadMonitor = require('../../../../scripts/playwright-load-monitor.cjs') as {
...
test.describe('Playwright load monitor script', { tag: '@svc-internal' }, () => {
  ...
  test('writes timeline event JSONL records for Jenkins stage markers', () => {
    ...
    childProcess.execFileSync('node', [path.join(process.cwd(), 'scripts/playwright-load-event.cjs'), eventsFile, 'API', 'start']);
