...
test.describe('Dynamic user support unit tests: orchestration flows', { tag: '@svc-internal' }, () => {
  ...
  test('ProfessionalUserUtils defaults service-token microservice name to xui_webapp when env is unset', async () => {
    ...
    try {
      ...
      const token = await (
        utils as unknown as {
          resolveServiceToken: (token?: string, stripBearerPrefix?: boolean) => Promise<string>;
        }
      ).resolveServiceToken(undefined, true);
  ...
  test('ProfessionalUserUtils normalises an email-shaped assignment UI user before bootstrapping the manage-org session path', async () => {
    ...
    try {
      const utils = new ProfessionalUserUtils(
        ...
        {
          ensureUiStorageStateForUser: async (
          ...
          resolveUiStoragePathForUser: (userIdentifier?: string) => {
            const resolvedUserIdentifier = userIdentifier ?? '';
            resolvedStorageUsers.push(resolvedUserIdentifier);
            return `/tmp/${resolvedUserIdentifier}.json`;
          },
          newApiContext: async ({ storageState, baseURL }: { storageState?: string; baseURL?: string }) => {
