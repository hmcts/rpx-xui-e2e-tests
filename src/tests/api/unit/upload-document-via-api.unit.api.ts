...
test.describe('Document upload helper unit tests', { tag: '@svc-internal' }, () => {
  ...
  test('uploadDocumentViaApi fails fast when the XSRF cookie never appears', async () => {
    ...
    expect(requestGets.filter((entry) => entry.endsWith('/auth/login'))).toHaveLength(3);
    expect(requestGets.filter((entry) => entry.endsWith('/auth/isAuthenticated'))).toHaveLength(6);
