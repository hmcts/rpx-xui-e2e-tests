#!/usr/bin/env node
/**
 * Copies the Odhin report (and optional coverage) into a publish-friendly folder.
 * Best-effort: never fails the build if copying/injection fails.
 */
const fs = require("fs");
const path = require("path");

const source = path.resolve("test-results", "odhin-report");
const targetRoot = path.resolve("functional-output", "tests", "playwright-api");
...
function findCoverageIndex(rootDir) {
  ...
  return fs.existsSync(preferred) ? preferred : undefined;
}
...
function injectCoverageLink(reportFolder, relativeCoveragePath, totals) {
  ...
  const summaryLines = totals
    ? [
        `Statements   : ${formatPct(totals.statements)} (${totals.statements?.covered ?? 0}/${totals.statements?.total ?? 0})`,
        `Branches     : ${formatPct(totals.branches)} (${totals.branches?.covered ?? 0}/${totals.branches?.total ?? 0})`,
        `Functions    : ${formatPct(totals.functions)} (${totals.functions?.covered ?? 0}/${totals.functions?.total ?? 0})`,
        `Lines        : ${formatPct(totals.lines)} (${totals.lines?.covered ?? 0}/${totals.lines?.total ?? 0})`
      ]
    : [];
          ...
          <div class="row ms-3 me-3">
            <div class="col-12">
              <div class="mt-3 mb-3 odhin-thin-border dashboard-block">
                ...
                ${
                  summaryLines.length
                    ? `<pre class="text-secondary-emphasis small mb-2 ps-2">
=============================== Coverage summary ===============================
${summaryLines.join("\n")}
===============================================================================
</pre>`
...
<div id="TabCoverage" style="display: none" class="main-tabcontent">
  ...
  files.forEach((file) => {
    ...
    try {
      ...
      html = injectTabButton(html, tabButton);
...
function formatPct(entry) {
  return typeof entry?.pct === "number" ? `${entry.pct.toFixed(2)}%` : "n/a";
}
...
function injectTabButton(html, buttonHtml) {
  const start = html.indexOf('<div class="tab">');
  if (start === -1) {
    return html;
  }
  ...
  return `${html.slice(0, end)}${buttonHtml}\n${html.slice(end)}`;
}
