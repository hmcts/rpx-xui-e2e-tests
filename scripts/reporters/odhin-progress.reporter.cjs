class OdhinProgressReporter {
  constructor(options = {}) {
    ...
    this.hardTimeoutMs =
      Number.isFinite(Number(options.hardTimeoutMs)) &&
      Number(options.hardTimeoutMs) >= 5000
        ? Number(options.hardTimeoutMs)
        : 0;
    this.startedAt = 0;
    this.tick = 0;
  ...
  startProgress() {
    ...
    const planned =
      this.totalPlannedTests > 0
        ? `plannedTests=${this.totalPlannedTests}`
        : "plannedTests=unknown";
    process.stdout.write(
      `[odhin-progress] All tests finished. Finalizing Odhin report... (${planned})\n`,
    );
    ...
    this.timer = setInterval(() => {
      ...
      process.stdout.write(
        `[odhin-progress] Finalizing Odhin report${dots} elapsed=${elapsedSeconds}s\n`,
      );
    }, this.intervalMs);
  ...
  onExit() {
    ...
    if (this.progressStarted) {
      const elapsedSeconds = this.startedAt
        ...
        : 0;
      process.stdout.write(
        `[odhin-progress] Odhin report completed in ${elapsedSeconds}s.\n`,
      );
    }
  }
}
