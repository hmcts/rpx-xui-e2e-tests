...
class FlakeGateReporter {
  constructor() {
    ...
    this.maxFlakyTests = Number.isFinite(Number(process.env.PW_MAX_FLAKY_TESTS))
      ? Number(process.env.PW_MAX_FLAKY_TESTS)
      : 20;
    this.maxFlakyRate = Number.isFinite(Number(process.env.PW_MAX_FLAKY_RATE))
      ? Number(process.env.PW_MAX_FLAKY_RATE)
      : 0.2;
    this.reportOnly =
      String(process.env.PW_FLAKE_GATE_REPORT_ONLY || "").toLowerCase() ===
      "true";
  }
  ...
  onEnd() {
    ...
    const flakyCount = finalOutcomes.filter(
      (item) =>
        item.outcome === "flaky" ||
        (item.status === "passed" && item.retry > 0),
    ).length;
    const retriedPassCount = finalOutcomes.filter(
      (item) => item.status === "passed" && item.retry > 0,
    ).length;
    ...
    const breachedByCount = flakyCount > this.maxFlakyTests;
    const breachedByRate = flakyRate > this.maxFlakyRate;
    const gateBreached = breachedByCount || breachedByRate;
    const summary = [
      ...
      `[flake-gate] breached=${gateBreached}`,
    ].join("\n");

    process.stdout.write(`${summary}\n`);

    if (gateBreached && !this.reportOnly) {
      process.stderr.write(
        `[flake-gate] Gate breached (count=${flakyCount}, rate=${(flakyRate * 100).toFixed(2)}%). Failing run.\n`,
      );
      process.exitCode = 1;
    }
  }
