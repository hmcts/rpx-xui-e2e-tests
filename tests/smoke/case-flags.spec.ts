...
const CASE_FLAGS_CASE_ID =
  process.env.CASE_FLAGS_CASE_ID ?? "1747043572209027";
const CASE_FLAGS_ENABLED =
  process.env.CASE_FLAGS_ENABLED === "true";

test.describe("@smoke @case-flags Case flags", () => {
  // eslint-disable-next-line playwright/no-skipped-test
  test.skip(!CASE_FLAGS_ENABLED, "Case flags disabled for this environment");
  test.beforeEach(async ({ loginAs }) => {
    await loginAs(CASE_FLAGS_USER);
  });

  test("Banner count matches table entries", async ({ page, config }) => {
    await gotoCaseDetails(page, config.urls.manageCaseBaseUrl);

    const bannerCount = await getActiveBannerCount(page);
    await expect(
      page.getByText(new RegExp(`There are ${bannerCount} active flags`))
    ).toBeVisible();
...
async function countActiveFlagRows(page: Page): Promise<number> {
  const statusCells = page.locator(
    "ccd-case-flag-table tbody tr td.cell-flag-status"
  );
