...
export type ApiUserRole = "solicitor" | "caseOfficer_r1" | "caseManager" | "judge";

const baseUrl = stripTrailingSlash(config.baseUrl);
const storageRoot = path.resolve(process.cwd(), "test-results", "api", "storage-states");
const storagePromises = new Map<string, Promise<string>>();

const logger = createLogger({
  serviceName: "rpx-xui-api-auth",
  format: process.env.LOG_FORMAT ?? (process.env.CI ? "json" : "pretty"),
});

export const hasCredentials = (role: ApiUserRole): boolean => {
  const envUsers = config.users[config.testEnv as keyof typeof config.users] ?? {};
  const record = envUsers[role];
  return Boolean(record?.e && record?.sec);
};
...
export function shouldAutoInjectXsrf(): boolean {
...
async function tryTokenBootstrap(
  ...
  const microservice =
    process.env.S2S_MICROSERVICE_NAME ?? process.env.MICROSERVICE ?? "xui_webapp";
  ...
  try {
    ...
    const isAuth = authCheck.status() === 200 ? Boolean(await safeJson(authCheck)) : false;
...
async function createStorageStateViaForm(
  ...
  try {
    ...
    throw new Error(`Failed to login as ${role}: ${message}`);
  } finally {
    await context.dispose();
  }
}
...
function getCredentials(role: ApiUserRole): { username: string; password: string } {
  ...
  if (!userConfig) {
    throw new Error(
      `No credentials configured for role "${role}" in environment "${config.testEnv}". Set ${role
        .toUpperCase()
        .replace(/[^A-Z0-9]/g, "_")}_USERNAME / ${role
        .toUpperCase()
        .replace(/[^A-Z0-9]/g, "_")}_PASSWORD environment variables.`,
    );
  }
...
async function tryReadState(
  storagePath: string,
): Promise<{ cookies?: { name?: string; value?: string }[] } | undefined> {
  try {
    ...
    if (
      ...
      Array.isArray((parsed as { cookies?: { name?: string; value?: string }[] }).cookies)
    ) {
      return parsed as { cookies?: { name?: string; value?: string }[] };
    }
  } catch {
