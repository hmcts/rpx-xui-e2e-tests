...
import { expectRoleAssignmentShape } from "./utils/assertions.ts";
import { seedRoleAccessCaseId } from "./utils/role-access.ts";
import { RoleAssignmentContainer } from "./utils/types.ts";
...
test.describe("@api global search", () => {
  test("lists available services", async ({ apiClient }) => {
    const response = await withRetry(
      () =>
        apiClient.get<{ serviceId: string; serviceName: string }[]>("api/globalSearch/services", {
          throwOnError: false,
        }),
  ...
  test("returns results payload or guarded status", async ({ apiClient }) => {
    const response = await apiClient.post<GlobalSearchResponse>("api/globalSearch/results", {
...
test.describe("@api ref data and supported jurisdictions", () => {
  test("wa-supported jurisdictions", async ({ apiClient }) => {
    const res = await apiClient.get<string[]>("api/wa-supported-jurisdiction", {
      throwOnError: false,
    });
  ...
  test("staff-supported jurisdictions", async ({ apiClient }) => {
    const res = await apiClient.get<string[]>("api/staff-supported-jurisdiction", {
      throwOnError: false,
    });
    expectStatus(res.status, StatusSets.guardedBasic);
    if (res.status === 200 && Array.isArray(res.data) && res.data.length > 0) {
  ...
  test("locations endpoint returns list or guarded status", async ({ apiClient }) => {
    await withXsrf("solicitor", async (headers) => {
      const res = await apiClient.get<{ epimms_id?: string }[]>("api/locations", {
        headers,
        throwOnError: false,
      });
      expectStatus(res.status, StatusSets.guardedBasic);
  ...
  test("staff-ref-data endpoint responds", async ({ apiClient }) => {
    const res = await apiClient.post<{ staff?: { known_as?: string; email_id?: string }[] }>(
...
test.describe("@api role access / AM", () => {
  let roleAccessCaseId = ROLE_ACCESS_CASE_ID ?? "1234567890123456";
  const hasCaseOfficer =
    !!config.users?.[config.testEnv as keyof typeof config.users]?.caseOfficer_r1;
  ...
  test("roles/access-get responds", async ({ apiClient }) => {
    ...
    if (res.status === 200) {
      if (Array.isArray(res.data) && res.data.length > 0) {
        expectRoleAssignmentShape(res.data[0]);
      } else if (
        Array.isArray(res.data?.roleAssignmentResponse) &&
        res.data.roleAssignmentResponse.length > 0
      ) {
        expectRoleAssignmentShape(res.data.roleAssignmentResponse[0]);
      } else {
  ...
  test("allocate-role/valid-roles responds", async ({ apiClient }) => {
    const res = await apiClient.post<{ roleId: string; roleName: string }[]>(
  ...
  test("roles/access-get-by-caseId responds with roles when present", async ({ apiClient }) => {
    ...
    expectStatus(res.status, [200, 400, 401, 403, 404, 500]);
    const data: RoleAssignmentContainer = res.data;
    const roleAssignments = Array.isArray(data?.roleAssignmentResponse)
      ? data.roleAssignmentResponse
      : [];
    if (res.status === 200 && roleAssignments.length > 0) {
      expectRoleAssignmentShape(roleAssignments[0]);
    }
  });
  ...
  test("role access confirm returns guarded status for stale session", async () => {
    ...
    const state = JSON.parse(raw) as { cookies?: { expires?: number }[] };
