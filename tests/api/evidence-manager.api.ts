...
import { ensureStorageState, getStoredCookie } from "./auth.ts";
import { config } from "./config.ts";
import { EM_DOC_ID } from "./data/testIds";
import { test, expect } from "./fixtures.ts";
import { expectStatus, withXsrf } from "./utils/apiTestUtils.ts";
import { expectAnnotationShape, expectBookmarkShape } from "./utils/assertions.ts";
import { AnnotationPayload, BookmarkPayload } from "./utils/types.ts";
...
test.describe("@api evidence manager & documents", () => {
  ...
  test("returns document binary with XSRF", async ({ apiClient }) => {
    await withXsrf("solicitor", async (headers) => {
      ...
      if (res.status === 200) {
        expect(res.data?.byteLength ?? 0).toBeGreaterThan(0);
      }
    });
  });
  ...
  test("annotations metadata guarded by session", async ({ apiClient, anonymousClient }) => {
    const anon = await anonymousClient.get(`em-anno/metadata/${sharedDocId}`, {
      throwOnError: false,
    });
  ...
  test("rejects annotation mutation with invalid payload", async ({ apiClient }) => {
    await withXsrf("solicitor", async (headers) => {
      const badPayload = { id: randomUUID(), rectangles: [] };
      const res = await apiClient.put("em-anno/annotations", {
        data: badPayload,
        headers,
        throwOnError: false,
  ...
  test("bookmarks lifecycle", async ({ apiClient }) => {
    await withXsrf("solicitor", async (headers) => {
      const listRes = await apiClient.get<any[]>(`em-anno/${sharedDocId}/bookmarks`, {
        headers,
        throwOnError: false,
      });
...
async function fetchUserId(apiClient: any): Promise<string | undefined> {
  const res = await apiClient.get<{ userInfo?: { uid?: string; id?: string } }>(
    "api/user/details",
    {
      throwOnError: false,
    },
...
async function uploadSyntheticDoc(): Promise<string> {
  ...
  try {
    ...
    if (res.ok()) {
      ...
      const id =
        body?.documents?.[0]?.originalDocumentId ??
        body?.documents?.[0]?.documentId ??
        body?.documents?.[0]?.id;
      if (id) {
        await ctx.dispose();
        return id;
