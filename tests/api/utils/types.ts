...
export function isTaskList(payload: unknown): payload is TaskListResponse {
  return (
    ...
    Array.isArray((payload as Partial<TaskListResponse>).tasks)
  );
}

const hasArrayProperty = (value: unknown, property: string): value is Record<string, unknown[]> =>
  typeof value === "object" &&
  value !== null &&
...
export function extractCaseShareEntries(
  ...
  if (hasArrayProperty(payload, property)) {
    return payload[property];
  }

  const nested = payload.payload;
  if (nested && typeof nested === "object" && hasArrayProperty(nested, property)) {
