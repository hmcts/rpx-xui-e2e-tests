...
export function buildTaskSearchRequest(
  ...
  const searchParameters: { key: string; operator: "IN"; values: string[] }[] = [];
...
export async function seedTaskId(
  ...
  for (const candidate of candidateStates) {
    ...
    const response = await apiClient.post<TaskListResponse>("workallocation/task", {
      data: body,
      throwOnError: false,
    });
    const tasks = response.data?.tasks ?? [];
    if (response.status === 200 && Array.isArray(tasks) && tasks.length > 0) {
      const id = tasks[0]?.id;
      if (id) {
